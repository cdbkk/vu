...
const GHOSTTY_ENV: &str = "VU_GHOSTTY_SOURCE_DIR";
const GHOSTTY_INITIAL_OUTPUT_REQUIRE_ENV: &str = "VU_REQUIRE_GHOSTTY_INITIAL_OUTPUT";
const GHOSTTY_VT_TARGET_ENV: &str = "VU_GHOSTTY_VT_TARGET";
...
fn main() {
    ...
    // toggling VU_STUB_GHOSTTY_VT off wouldn't re-build libghostty-vt
    ...
    println!("cargo:rerun-if-env-changed=VU_STUB_GHOSTTY_VT");
    println!("cargo:rerun-if-env-changed=VU_SKIP_GHOSTTY_VT");
    println!("cargo:rerun-if-env-changed=VU_GHOSTTY_VT_SIMD");
    ...
    match target_os.as_str() {
        ...
        other => {
            println!(
                "cargo:warning=vu-ghostty: skipping native build on target_os={other} \
...
fn build_macos() {
    ...
    println!(
        "cargo:rustc-env=VU_GHOSTTY_RESOURCES_DIR={}",
        ghostty_dir.join("zig-out/share/ghostty").display()
    );
}

fn ghostty_optimize() -> String {
    env::var("VU_GHOSTTY_OPTIMIZE").unwrap_or_else(|_| "ReleaseFast".to_string())
}
...
fn build_vt_backend(target_os: &str) {
    ...
    if target_os == "windows" && env::var_os("VU_STUB_GHOSTTY_VT").is_some() {
        ...
        println!(
            "cargo:warning=VU_STUB_GHOSTTY_VT set — linking stub C implementations \
    ...
    if env::var_os("VU_SKIP_GHOSTTY_VT").is_some() {
        ...
        println!(
            "cargo:warning=VU_SKIP_GHOSTTY_VT set — skipping libghostty-vt build. \
             A subsequent `cargo build` will fail to link unless a static \
             library is provided manually.{}",
            if target_os == "windows" {
                " Consider VU_STUB_GHOSTTY_VT=1 if you want a linkable placeholder."
            } else {
                ""
            }
        );
    ...
    match zig_probe {
        ...
        Ok(output) => {
            ...
            panic!(
                "\n\n========================================================\n\
                 vu-ghostty: `{bin} version` exited with status {status}.\n\
                 stderr:\n{stderr}\n\
                 ========================================================\n",
    ...
    // the probe via `VU_GHOSTTY_VT_STEP` (step name) or
    // `VU_GHOSTTY_VT_EMIT_OPTION=1` (force the option-based path).
    ...
    // `VU_GHOSTTY_OPTIMIZE=Debug` opt back in for VT-debugging.
    //
    ...
    let simd_on = env::var("VU_GHOSTTY_VT_SIMD")
        ...
        if !retry_status.success() {
            panic!(
                "zig build {:?} failed after package-cache prefetch; see output above.\n\
                 ...
                 bump GHOSTTY_REV in crates/vu-ghostty/build.rs or set\n\
                 VU_GHOSTTY_VT_STEP to the correct step name.",
                invocation
            );
        }
    }
    ...
    // the import stub would leave `vu-app.exe` dependent on
...
///
/// Priority:
///   1. `VU_GHOSTTY_VT_STEP` env var -> single named step.
...
fn pick_vt_invocation(
    ...
    if let Some(val) = env::var_os("VU_GHOSTTY_VT_STEP") {
    ...
    panic!(
        "\n========================================================\n\
         vu-ghostty: couldn't find a libghostty-vt build knob in this\n\
         ...
         `zig build -h` output:\n{help}\n\n\
         To work around: bump GHOSTTY_REV in crates/vu-ghostty/build.rs,\n\
         or set VU_GHOSTTY_VT_STEP to an exact step name from the list\n\
         above.\n\
         ========================================================\n",
        help = help_text,
    );
...
fn zig_build_help_text(
    ...
    for attempt in 0..2 {
        ...
        panic!(
            "\n========================================================\n\
             vu-ghostty: `zig build -h` failed while probing the\n\
...
fn resolve_ghostty_source() -> PathBuf {
    if let Some(source_dir) = env::var_os(GHOSTTY_ENV) {
        return PathBuf::from(source_dir)
            .canonicalize()
            .expect("VU_GHOSTTY_SOURCE_DIR points to a missing directory");
    }
...
fn patchable_ghostty_source(source: PathBuf) -> PathBuf {
    ...
    // Never patch a caller-provided checkout in place. `VU_GHOSTTY_SOURCE_DIR`
    // is for local upstream study; Vu's embedding extension is applied to a
    ...
    copy_dir_filtered(&source, &patched).unwrap_or_else(|err| {
        panic!(
            "failed to copy VU_GHOSTTY_SOURCE_DIR from {} to {}: {err}",
            source.display(),
            patched.display()
        )
    });
...
fn apply_embedded_initial_output_patch(ghostty_dir: &Path) -> bool {
    if let Err(err) = try_apply_embedded_initial_output_patch(ghostty_dir) {
        if env_flag_enabled(GHOSTTY_INITIAL_OUTPUT_REQUIRE_ENV) {
            panic!(
                "vu-ghostty: embedded initial_output restore hook is required but could not be applied: {err}\n\
                 Rebase Vu's Ghostty embedding patch against GHOSTTY_REV={GHOSTTY_REV}, or unset \
                 {GHOSTTY_INITIAL_OUTPUT_REQUIRE_ENV} for a local best-effort build."
            );
        }
        println!("cargo:warning=vu-ghostty: embedded initial_output restore hook disabled: {err}");
        return false;
    }
...
fn try_apply_embedded_initial_output_patch(ghostty_dir: &Path) -> Result<(), String> {
    ...
    patch_file_once(
        &embedded,
        "Vu: macOS may deny directory open/stat preflight for privacy-protected cwd",
        &[(
    ...
    println!("cargo:rustc-cfg=vu_ghostty_embedded_initial_output");
...
fn patch_file_once(path: &Path, marker: &str, replacements: &[(&str, &str)]) -> Result<(), String> {
    ...
    for (from, to) in replacements {
        if !text.contains(from) {
            return Err(format!(
                "failed to patch {}: expected anchor not found while applying Vu embedding extension",
                path.display()
            ));
        }
...
fn write_file_atomic(path: &Path, text: &str) -> Result<(), String> {
    let tmp = path.with_extension(format!("vu-patch-{}.tmp", std::process::id()));
...
fn writable_dir(path: &Path) -> bool {
    let probe = path.join(".vu_ghostty_write_test");
...
fn prefetch_zig_dependencies(zig_bin: &OsStr, root: &Path, zig_global_cache_dir: Option<&Path>) {
    let Some(cache_dir) = resolve_zig_global_cache_dir(zig_bin, zig_global_cache_dir) else {
        println!("cargo:warning=vu-ghostty: could not resolve Zig global cache dir for prefetch");
        return;
    };
    ...
    while let Some(zon_file) = zon_queue.pop_front() {
        ...
        for url in extract_zon_urls(&text) {
            ...
            match prefetch_zig_url(zig_bin, &url, zig_global_cache_dir) {
                ...
                None => {
                    failed += 1;
                    println!("cargo:warning=vu-ghostty: failed to prefetch Zig package {url}");
                }
            }
        }
    }

    println!("cargo:warning=vu-ghostty: prefetched {fetched} Zig package(s), {failed} failed");
}
...
fn prefetch_zig_url(
    ...
    match curl_status {
        ...
        Ok(status) => {
            let _ = fs::remove_file(&tmp);
            println!("cargo:warning=vu-ghostty: curl exited with status {status} for {url}");
            None
        }
        Err(err) => {
            let _ = fs::remove_file(&tmp);
            println!("cargo:warning=vu-ghostty: failed to run curl for {url}: {err}");
            None
        }
    }
}
...
fn zig_fetch_path(
    ...
    if !output.status.success() {
        ...
        println!(
            "cargo:warning=vu-ghostty: zig fetch {} failed: {stderr}",
            path.display()
        );
        return None;
    }
