...
fn perf_trace_enabled() -> bool {
    ...
    *ENABLED.get_or_init(|| {
        std::env::var_os("VU_GHOSTTY_PROFILE").is_some_and(|v| !v.is_empty() && v != "0")
    })
}
...
impl ConPty {
        ...
        F: FnMut(&[u8]) + Send + 'static,
    {
        ...
        // handles. This matters when vu itself is launched with
        // redirected logs (`*> vu-profile.log`): inheriting those
...
/// `VU_SHELL` (future: a config-file field). Otherwise we try to honor
...
pub fn default_shell_command() -> String {
    let command = if let Some(cmd) = std::env::var("VU_SHELL")
        .ok()
...
fn powershell_cwd_integration_script_path() -> Option<PathBuf> {
    let dir = std::env::temp_dir().join("vu-terminal");
    ...
    let path = dir.join("vu-powershell-cwd-integration.ps1");
...
function global:__VuTerminalEmitOsc7 {
    try {
...
if (-not $global:__VuTerminalPromptHookInstalled) {
    $global:__VuTerminalPromptHookInstalled = $true
    $global:__VuTerminalOriginalPrompt = (Get-Command prompt -CommandType Function -ErrorAction SilentlyContinue).ScriptBlock
    function global:prompt {
        global:__VuTerminalEmitOsc7
        if ($global:__VuTerminalOriginalPrompt) {
            & $global:__VuTerminalOriginalPrompt
        } else {
...
mod tests {
    ...
    fn wraps_powershell_shells_with_cwd_integration() {
        let script = Path::new(r"C:\Users\me\AppData\Local\Temp\Vu Terminal\vu-cwd.ps1");
        ...
        assert!(wrapped.contains(r#""C:\Users\me\AppData\Local\Temp\Vu Terminal\vu-cwd.ps1""#));
    }

    #[test]
    fn preserves_existing_noexit_when_wrapping_powershell() {
        let script = Path::new(r"C:\Temp\vu-cwd.ps1");
