...
fn perf_trace_verbose() -> bool {
    static ENABLED: OnceLock<bool> = OnceLock::new();
    *ENABLED.get_or_init(|| {
        std::env::var_os("VU_GHOSTTY_PROFILE_VERBOSE").is_some_and(|v| !v.is_empty() && v != "0")
    })
}
    ...
    pub fn new_with_write_pty(
        ...
        let mut callback_state = write_pty.map(|write_pty| {
            Box::new(VtCallbackState {
                write_pty,
                enquiry_response: b"vu".to_vec().into_boxed_slice(),
        ...
        let enable_render_state = std::env::var("VU_GHOSTTY_VT_RENDER_STATE")
        ...
        if enable_render_state {
            ...
            log::warn!(
                "VtScreen::new: render_state disabled via \
                 VU_GHOSTTY_VT_RENDER_STATE=0 — terminal output will \
...
mod tests {
    ...
    fn vt_screen_reports_osc7_current_dir() {
        ...
        screen.feed(b"\x1b]7;file:///tmp/vu-vt-cwd\x07");

        assert_eq!(screen.current_dir().as_deref(), Some("/tmp/vu-vt-cwd"));
    }
    ...
    fn vt_screen_reports_windows_osc7_current_dir() {
        ...
        screen.feed(b"\x1b]7;file:///C:/Users/WeyGu/dev/vu-terminal\x07");

        assert_eq!(
            screen.current_dir().as_deref(),
            Some("C:\\Users\\WeyGu\\dev\\vu-terminal")
        );
    }
    ...
    fn vt_screen_reports_localhost_windows_osc7_current_dir() {
        ...
        screen.feed(b"\x1b]7;file://localhost/C:/Users/WeyGu/dev/vu-terminal\x07");
    ...
    fn vt_screen_reports_split_osc7_current_dir() {
        ...
        screen.feed(b"\x1b]7;file:///home/me/vu");
        screen.feed(b"%20terminal\x1b\\");

        assert_eq!(
            screen.current_dir().as_deref(),
            Some("/home/me/vu terminal")
        );
    }
