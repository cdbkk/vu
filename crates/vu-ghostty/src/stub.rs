//! Non-macOS stub backend for vu-ghostty.
//!
...
//! `CommandRecord`) so cross-platform code in `vu-app` compiles without
...
//!
//! Keeping the stubs in `vu-ghostty` (rather than per-platform crates)
