...
impl GhosttyConfigPatch {
    ...
    fn to_config_string(&self) -> String {
        ...
        // Disable ghostty shell-integration cursor override so vu's
    ...
    fn write_config_file(&self) -> Result<std::path::PathBuf, String> {
        let dir = std::env::temp_dir().join("vu-ghostty");
...
fn build_ghostty_config(patch: &GhosttyConfigPatch) -> Result<ffi::ghostty_config_t, String> {
    ...
    // Always load Vu's runtime config, even when no appearance patch fields
    // are present. Some terminal behavior is part of Vu's product default
...
fn ensure_resources_dir_env() {
    ...
    let resources_dir = installed_app_ghostty_resources_dir().or_else(|| {
        let resources_dir = option_env!("VU_GHOSTTY_RESOURCES_DIR")?;
...
impl GhosttyApp {
    ...
    pub fn new_surface(
        ...
        #[cfg(vu_ghostty_embedded_initial_output)]
        ...
        #[cfg(not(vu_ghostty_embedded_initial_output))]
...
mod tests {
    ...
    #[test]
    fn ghostty_config_always_includes_vu_shell_integration_features() {
    ...
    fn finds_ghostty_resources_inside_installed_app_bundle() {
        let (_cleanup, root) = temp_test_dir("vu-ghostty-resources-test");
        let app_root = root.join("vu Beta.app");
        ...
        let found = installed_app_ghostty_resources_dir_for_exe(&macos_dir.join("vu"));
    ...
    fn finds_ghostty_resources_when_terminfo_uses_letter_bucket() {
        let (_cleanup, root) = temp_test_dir("vu-ghostty-letter-terminfo-test");
        let app_root = root.join("vu Beta.app");
        let macos_dir = app_root.join("Contents/MacOS");
        let resources_dir = app_root.join("Contents/Resources/ghostty");
    ...
    fn ignores_installed_app_resources_when_bundled_terminfo_is_missing() {
        let (_cleanup, root) = temp_test_dir("vu-ghostty-missing-terminfo-test");
        let app_root = root.join("vu.app");
        let macos_dir = app_root.join("Contents/MacOS");
        let resources_dir = app_root.join("Contents/Resources/ghostty");
        std::fs::create_dir_all(&macos_dir).unwrap();
        std::fs::create_dir_all(&resources_dir).unwrap();
