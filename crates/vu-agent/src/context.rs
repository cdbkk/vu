        ...
        match event {
            PaneRuntimeEvent::PaneCreated { startup_command } => {
                let summary = startup_command
                    .as_ref()
                    .map(|command| {
                        format!("vu created this pane with startup command `{command}`")
                    })
...
/// This is what makes vu's agent smarter than a generic chatbot —
...
pub struct TerminalContext {
    ...
    /// Recent vu-originated actions on the focused pane.
...
fn focused_screen_assessment(
    ...
    let summary = match (has_prompt, has_htop) {
        ...
        _ if has_tmux_like && has_prompt => {
            "The current visible screen looks like a tmux workspace sitting at a prompt-like line, but vu does not yet have native tmux control or authoritative foreground-process proof."
        }
        _ if has_tmux_like => {
...
impl TerminalContext {
    ...
    pub fn to_system_prompt(&self) -> String {
        ...
        prompt.push_str(
            "You are vu, a terminal AI assistant with full access to the user's terminal environment.\n\n\
             ## Decision framework\n\
             You are an orchestrator with direct API access to the terminal.\n\n\
             ### Keystroke categories — know the difference\n\
             1. **vu application shortcuts** (Cmd+T, Cmd+W, Cmd+N): These control the vu app itself.\n\
             ...
             - REMOTE TMUX WORKSPACE PREPARATION (\"connect to haswell and prepare tmux\", \"ensure tmux session vu-bench on host X\") → ensure_remote_tmux_workspace. Prefer this over manually chaining ensure_remote_shell_target + terminal_exec + tmux_list_targets when you only need the tmux session/bootstrap fact.\n\
             ...
             - INTERACTIVE AGENT-CLI FOLLOW-UP (Codex / Claude Code / OpenCode already running in a known pane or tmux target) → agent_cli_turn. Prefer this over raw send_keys or tmux_send_keys so vu can wait for the interactive target to settle and return a fresh snapshot before you continue.\n\
             ...
             ## Tools\n\n\
             <tools>\n\
             - terminal_exec: Run a command visibly in a vu pane. Requires `exec_visible_shell` capability.\n\
               `pane_index` addresses a vu pane from list_panes — not a tmux pane/window or editor buffer.\n\
             ...
             - send_keys: Send keystrokes to a pane's PTY. Use for:\n\
               ...
               NEVER use to simulate vu application shortcuts (Cmd+T, Cmd+W).\n\n\
             ...
             - resolve_work_target: Choose the best vu pane or tmux target for shell work, tmux work, or agent CLI interaction using the typed control plane. When it returns a vu pane, carry its `pane_id` into the next tool call.\n\
             ...
             - tmux_capture_pane: Capture the content of a specific tmux pane target without confusing it with the outer vu pane.\n\
             ...
             - tmux_ensure_agent_target: Reuse or create a tmux target for Codex CLI, Claude Code, or OpenCode. This stays in tmux control; it does not imply a native Codex/OpenCode attachment unless vu explicitly proves one.\n\
             ...
             - Action history is causal evidence, not foreground truth. Use recent vu actions to understand how a pane was reached, but rely on current probes and control capabilities before acting.\n\
             - `runtime_stack` is current-only. `last_verified_shell_stack` describes the most recent shell frame vu verified, and may be historical.\n\
             ...
             - `<remote_workspaces>` is not a guess. It is vu's current host-workspace inventory for this tab, built from pane-local runtime facts and vu-managed SSH continuity.\n\
             - `<local_workspaces>` is vu's current local coding-workspace inventory for this tab. Use it to reuse local shell and local agent-cli panes for the same project path instead of opening duplicates.\n\
             ...
             - Addressing is layered. A vu pane index ≠ tmux pane id ≠ tmux window index ≠ editor buffer.\n\
...
mod tests {
    ...
    fn runtime_state_detects_codex_from_visible_screen() {
        let observation = PaneObservationFrame {
            title: Some("vu-bench-codex-git".to_string()),
            cwd: Some("/tmp".to_string()),
            recent_output: vec![
                "╭────────────────────────────────────────────╮".to_string(),
