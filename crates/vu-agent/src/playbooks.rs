...
1. resolve_work_target when you need vu to choose the best pane or tmux workspace first
...
4. tmux_capture_pane to inspect the chosen tmux pane without confusing it with the outer vu pane
5. tmux_ensure_shell_target when you need a safe shell pane for work
6. tmux_shell_turn when you need one deterministic shell command inside an existing tmux shell target
7. tmux_ensure_agent_target when you want to reuse or create a Codex CLI, Claude Code, or OpenCode tmux target
8. tmux_run_command when you need a fresh shell, a dedicated work window, or another long-running target
9. tmux_send_keys to a specific tmux pane target
...
reaches tmux, NOT a vu application shortcut, but it is still a lower-fidelity path than tmux-native control.
...
- Run one shell-lane step inside an existing tmux shell target: tmux_shell_turn(pane_index=..., target=\"%17\", command=\"python3 -m unittest -q\")
...
4. If native control is available, prefer tmux_shell_turn to send the heredoc through the target shell pane. Otherwise use send_keys in the outer pane. Write the file using heredoc:
...
4. Prefer tmux_shell_turn for deterministic shell commands in the specific shell pane target. Use tmux_send_keys only when you need raw interactive keys. Use outer-pane send_keys only when native control is unavailable
...
- Prefer ensure_local_coding_workspace(agent_name=\"codex\" | \"claude\" | \"opencode\", cwd=\"...\") when you need both the local coding CLI and its paired shell workspace.
- Use ensure_local_agent_target(agent_name=\"codex\" | \"claude\" | \"opencode\", cwd=\"...\") when you only need to reuse or launch the local coding CLI itself.
- Use agent_cli_turn(agent_name=\"codex\" | \"claude\" | \"opencode\", prompt=\"...\") for follow-up prompts once the interactive target already exists, so vu can wait for the CLI to settle before the next shell/test step.
