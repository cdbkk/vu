...
use crate::hook::{VuHook, ToolApprovalDecision};
use crate::tools::{
...
mod tests {
    ...
    fn codex_chatgpt_auth_sync_writes_rig_cache_without_clobbering_seen_source() {
        ...
        let target = root.join("vu").join("auth.json");
        std::fs::write(
            &source,
    ...
    fn codex_chatgpt_auth_sync_updates_when_codex_source_fingerprint_changes() {
        let root = std::env::temp_dir().join(format!(
            "vu-agent-codex-auth-update-test-{}-{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .expect("system clock should be after unix epoch")
    ...
    fn codex_chatgpt_auth_sync_preserves_preexisting_unmanaged_auth() {
        let root = std::env::temp_dir().join(format!(
            "vu-agent-codex-auth-unmanaged-test-{}-{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .expect("system clock should be after unix epoch")
    ...
    fn codex_chatgpt_auth_sync_retires_ownership_after_local_auth_changes() {
        let root = std::env::temp_dir().join(format!(
            "vu-agent-codex-auth-local-test-{}-{}",
            std::process::id(),
            std::time::SystemTime::now()
                .duration_since(std::time::UNIX_EPOCH)
                .expect("system clock should be after unix epoch")
...
fn kimi_coding_headers() -> HeaderMap {
    ...
    // headers belong to the OAuth path, so Vu does not fabricate them here.
    headers.insert(
...
pub fn oauth_token_dir(kind: &ProviderKind) -> Option<PathBuf> {
    ...
    Some(vu_paths::app_config_dir().join("auth").join(provider_dir))
}
...
fn sync_codex_chatgpt_auth_from_file(
    ...
    // imported by an older Vu build before the provenance sidecar existed.
    ...
    if let Some(target_bytes) = target_bytes.as_deref() {
        ...
        if stable_fingerprint(target_bytes) != sync_state.source_fingerprint {
            // The cache changed after Vu imported it. Rig may have refreshed
...
/// Codex's `~/.codex/auth.json` into Vu's token directory.
///
...
/// never replaced unless its provenance marker proves that Vu previously
...
pub struct AgentConfig {
    ...
    ///
    /// Older Vu versions always serialized `provider`, including its default,
...
/// 0.40 makes the model-call budget exact, so retain Vu's shipped ceiling.
...
impl AgentProvider {
    ...
    fn build_openai_compatible_client_for(
        ...
        let api_key = configured_api_key.as_deref().unwrap_or("vu-local-keyless");
