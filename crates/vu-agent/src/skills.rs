...
///   1. Project-local: `<cwd>/.vu/skills/*/SKILL.md`
///   2. Global user:   `~/.config/vu/skills/*/SKILL.md`
///      (`~/.config/vu-terminal/skills/*/SKILL.md` on Windows)
///
...
mod tests {
    ...
    fn scan_finds_nested_skill_directories() {
        let root = std::env::temp_dir().join(format!("vu-skill-scan-{}", uuid::Uuid::new_v4()));
