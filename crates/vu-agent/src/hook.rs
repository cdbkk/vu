...
/// Hook that bridges Rig's agent loop into vu's event system.
///
...
/// ## Channel design
///
/// Each `VuHook` instance gets its own dedicated `approval_rx`.
...
impl<M: CompletionModel> AgentHook<M> for VuHook {
...
mod tests {
    ...
    #[tokio::test(flavor = "multi_thread")]
    async fn rig_streaming_loop_preserves_vu_tool_and_text_events() {
        ...
        let start_call_id = start_call_id.expect("Vu hook should emit the tool start");
