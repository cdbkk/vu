...
impl Tool for TerminalExecTool {
    ...
    fn description(&self) -> String {
        "Execute a command visibly in a vu terminal pane only when that pane's control_capabilities include exec_visible_shell. Use pane_id from list_panes for stable follow-up targeting; pane_index is only a positional snapshot. These selectors refer to a vu pane, not a tmux pane/window/editor target. For tmux, agent CLIs, and other TUIs, inspect first and use tmux-native tools or send_keys intentionally.".to_string()
    }
...
impl Tool for ShellExecTool {
    ...
    fn description(&self) -> String {
        "Execute a shell command in a hidden LOCAL background process on the vu workspace machine. Output is captured but not shown in the terminal. Never use this for remote SSH/tmux inspection or remote mutations.".to_string()
    }
...
fn agent_cli_native_attachment(agent_name: &str) -> (AgentCliNativeAttachmentState, String) {
    match canonical_agent_cli_name(agent_name) {
        Some("codex") => (
            ...
            "Codex has a separate app-server protocol, but vu can only use it when Codex is intentionally launched with app-server transport. A normal tmux Codex session is still controlled through tmux."
                .to_string(),
        ),
        Some("opencode") => (
            AgentCliNativeAttachmentState::LaunchIntegratedOnly,
            "OpenCode has a separate server mode, but vu can only use it when OpenCode is intentionally launched with an explicit server/port. A normal tmux OpenCode session is still controlled through tmux."
                .to_string(),
        ),
        Some("claude") => (
            AgentCliNativeAttachmentState::Unavailable,
            "Claude Code does not expose a separate vu-native control attachment here. Inside tmux it remains an interactive tmux target."
                .to_string(),
        ),
        _ => (
            AgentCliNativeAttachmentState::Unavailable,
            "This agent CLI does not have a proven vu-native attachment. Treat it as an interactive tmux target."
                .to_string(),
        ),
    }
}
...
fn pane_is_visible_agent_cli(pane: &PaneInfo, agent_name: Option<&String>) -> bool {
    ...
    // Vu-launched local agent panes can lose their visible header after a few
    // turns. Preserve that lane when Vu itself launched the CLI there and no
                ...
                candidates.push((
                    ...
                    make_visible_shell_candidate(
                        ...
                        if has_remote_fact {
                            ...
                            format!(
                                "This pane has visible shell execution and recent vu action history showing SSH startup for `{target}`."
                            )
                        },
                    ),
                ));
...
impl Tool for ListPanesTool {
    ...
    fn description(&self) -> String {
        "List all terminal panes currently open. Returns each pane's index, title, working directory, dimensions, current verified front-state, current runtime_stack, last_verified_runtime_stack, backend-support flags, typed shell context, recent vu actions, current visible-screen observation hints, and control state: address space, visible target, nested target_stack, explicit control_attachments, control channels, control capabilities, and notes. Use this before acting in tmux/TUI panes so you do not confuse a vu pane with a tmux pane or over-trust stale shell metadata. If a pane exposes query_tmux, exec_tmux_command, or send_tmux_keys, prefer tmux-native tools over outer-pane send_keys.".to_string()
    }
...
impl Tool for TmuxInspectTool {
    ...
    fn description(&self) -> String {
        "Inspect the tmux adapter state for a specific vu pane. Returns tmux adapter details only when tmux has been authoritatively detected for that pane, including the explicit reason native tmux pane/window control is or is not available. Use this when a pane's target_stack includes tmux.".to_string()
    }
    ...
    fn parameters(&self) -> serde_json::Value {
        serde_json::json!({
            ...
            "properties": {
                "pane_index": {
                    "type": "integer",
                    "description": "Target vu pane index from list_panes. Positional only."
                },
                "pane_id": {
                    "type": "integer",
                    "description": "Stable target pane id from list_panes. Prefer this for follow-up work."
...
impl Tool for TmuxListTool {
    ...
    fn description(&self) -> String {
        "List tmux panes across the current tmux session using a proven same-session tmux control anchor from the given vu pane. This is the preferred first step whenever list_panes shows tmux native control. Returns tmux session/window/pane ids plus pane_current_command and pane_current_path so the agent can target Codex CLI, Claude Code, OpenCode, or shell panes inside tmux explicitly.".to_string()
    }
    ...
    fn parameters(&self) -> serde_json::Value {
        serde_json::json!({
            ...
            "properties": {
                "pane_index": {
                    "type": "integer",
                    "description": "Target vu pane index from list_panes. Positional only; it can change."
                },
                "pane_id": {
                    "type": "integer",
...
impl Tool for TmuxCaptureTool {
    ...
    fn description(&self) -> String {
        "Capture scrollback from a tmux pane through a proven same-session tmux control anchor. This is the correct way to inspect a tmux pane's content without confusing it with the outer vu pane. If target is omitted, captures the current tmux pane of the anchor shell.".to_string()
    }
    ...
    fn parameters(&self) -> serde_json::Value {
        serde_json::json!({
            ...
            "properties": {
                ...
                "session_name": {
                    "type": ["string", "null"],
                    "description": "Optional tmux session to constrain shell-target reuse. Defaults to the pane's active tmux workspace session when vu already knows it."
                },
                "cwd": {
                    "type": ["string", "null"],
    ...
    Ok(EnsureLocalAgentTargetResult {
        ...
        reason: if let Some(pane) = continuity_hint {
            let cwd_note = args
                ...
                .unwrap_or_default();
            format!(
                "Pane {} was previously used for local {agent_name} work{}, but no live interactive {agent_name} target is currently proven there, so vu created a fresh dedicated local agent pane.",
                pane.index, cwd_note
            )
        } else if let Some(cwd) = args.cwd {
            format!(
                "No reusable local {agent_name} target matched `{cwd}`, so vu created a new local agent pane there."
            )
        } else {
            format!(
                "No reusable local {agent_name} target existed, so vu created a new local agent pane."
            )
        },
        native_attachment_state,
        native_attachment_note,
    ...
    fn parameters(&self) -> serde_json::Value {
        serde_json::json!({
            ...
            "properties": {
                ...
                "pane_index": {
                    "type": ["integer", "null"],
                    "description": "Optional vu pane index to constrain reuse checks. Positional only."
                },
                "pane_id": {
                    "type": ["integer", "null"],
    ...
    if shell_target.pane_id == agent_target.pane_id {
        ...
        if shell_target.pane_id == agent_target.pane_id {
            ...
            shell_target = EnsureLocalShellTargetResult {
                ...
                command,
                output: String::new(),
                reason: "The existing shell candidate was the same pane as the interactive agent target, so vu created a separate local shell pane to preserve the paired workspace.".to_string(),
            };
        }
    }
    ...
    fn parameters(&self) -> serde_json::Value {
        serde_json::json!({
            ...
            "properties": {
                ...
                "pane_index": {
                    "type": ["integer", "null"],
                    "description": "Optional vu pane index to bias or constrain agent-target reuse. Positional only."
                },
                "pane_id": {
                    "type": ["integer", "null"],
    ...
    fn parameters(&self) -> serde_json::Value {
        serde_json::json!({
            ...
            "properties": {
                ...
                "pane_index": {
                    "type": ["integer", "null"],
                    "description": "Optional vu pane index to constrain target selection. Positional only."
                },
                "pane_id": {
                    "type": ["integer", "null"],
                    "description": "Optional stable vu pane id to constrain target selection."
                },
                "cwd_contains": {
                    "type": ["string", "null"],
...
impl Tool for TmuxShellTurnTool {
    ...
    fn parameters(&self) -> serde_json::Value {
        serde_json::json!({
            ...
            "properties": {
                "pane_index": {
                    "type": ["integer", "null"],
                    "description": "Optional vu pane index that owns the tmux control anchor. Positional only."
                },
                "pane_id": {
                    "type": ["integer", "null"],
                    "description": "Optional stable vu pane id that owns the tmux control anchor."
                },
                "target": {
                    "type": "string",
    ...
    async fn call(&self, args: Self::Args) -> Result<Self::Output, Self::Error> {
        ...
        let resolved_pane = pane_query_list(&self.pane_tx)?
            ...
            .ok_or_else(|| {
                ToolError::CommandFailed(
                    "tmux shell turn completed, but vu could not resolve the owning pane afterward."
                        .to_string(),
                )
            })?;
...
impl Tool for ResolveWorkTargetTool {
    ...
    fn description(&self) -> String {
        "Resolve the best pane or tmux target for a specific kind of work using vu's typed control plane. Use this when multiple panes are open and you need to choose the right shell, the right tmux workspace, the best tmux shell pane, or a matching agent CLI target without re-deriving that logic from list_panes manually. When it returns a vu pane target, prefer its stable pane_id for follow-up work.".to_string()
    }
    ...
    fn parameters(&self) -> serde_json::Value {
        serde_json::json!({
            ...
            "properties": {
                ...
                "pane_index": {
                    "type": ["integer", "null"],
                    "description": "Optional vu pane index to constrain resolution to a single pane. Positional only."
                },
                "pane_id": {
                    "type": ["integer", "null"],
...
impl Tool for BatchExecTool {
    ...
    fn description(&self) -> String {
        "Execute commands across multiple vu panes in PARALLEL, but only when each pane's control_capabilities include exec_visible_shell. Prefer stable pane_id values from list_panes for follow-up work; pane_index is only positional.".to_string()
    }
...
impl Tool for WaitForTool {
    ...
    async fn call(&self, args: Self::Args) -> Result<Self::Output, Self::Error> {
        ...
        // for clean shutdown (same pattern as VuHook approval polling).
...
fn ensure_local_shell_target_impl(
    ...
    Ok(EnsureLocalShellTargetResult {
        ...
        reason: if let Some(cwd) = args.cwd {
            format!(
                "No reusable local shell target matched `{cwd}`, so vu created a new shell pane prepared for that directory."
            )
        } else {
            "No reusable local shell target existed, so vu created a new shell pane.".to_string()
        },
    })
}
    ...
    fn parameters(&self) -> serde_json::Value {
        serde_json::json!({
            ...
            "properties": {
                "cwd": {
                    "type": ["string", "null"],
                    "description": "Optional working directory the local shell should be prepared in. If no reusable shell matches it, vu creates a new pane and starts it there."
                },
                "pane_index": {
                    "type": ["integer", "null"],
                    "description": "Optional vu pane index to constrain reuse checks. Positional only."
...
fn ensure_remote_shell_target_impl(
        ...
        .filter_map(|pane| {
                ...
                .is_some_and(|target| target.to_ascii_lowercase().contains(&host_lower))
            {
                Some((
                    ...
                    RemoteShellMatchSource::ActionHistory,
                    format!(
                        "Pane {} has visible shell execution and recent vu action history showing SSH startup for `{}`.",
                        pane.index, args.host
                    ),
                ))
            } else {
    ...
    Ok(EnsureRemoteShellTargetResult {
        ...
        output: String::new(),
        reason: format!(
            "No reusable SSH pane for `{}` was found, so vu created a new pane to connect there.",
            args.host
        ),
    })
}
    ...
    fn parameters(&self) -> serde_json::Value {
        serde_json::json!({
            ...
            "properties": {
                ...
                "pane_index": {
                    "type": ["integer", "null"],
                    "description": "Optional vu pane index to constrain reuse checks to a specific pane. Positional only."
                },
                "pane_id": {
                    "type": ["integer", "null"],
...
impl Tool for EnsureRemoteTmuxWorkspaceTool {
    ...
    fn description(&self) -> String {
        "Prepare a reusable REMOTE tmux session from a host shell anchor. vu reuses or creates an SSH shell pane for the host, ensures the requested tmux session exists, and returns whether tmux-native control is immediately available from that same pane. Use this when you only need the tmux session/bootstrap fact. If you also need a clean tmux shell target for file work, prefer ensure_remote_tmux_shell_target instead of attaching tmux in the visible pane.".to_string()
    }
    ...
    fn parameters(&self) -> serde_json::Value {
        serde_json::json!({
            ...
            "properties": {
                ...
                "pane_index": {
                    "type": ["integer", "null"],
                    "description": "Optional vu pane index to constrain SSH reuse checks. Positional only."
                },
                "pane_id": {
                    "type": ["integer", "null"],
                    "description": "Optional stable vu pane id to constrain SSH reuse checks."
                },
                "location": {
                    "type": "string",
                    "enum": ["right", "down"],
...
impl Tool for EnsureRemoteTmuxShellTargetTool {
    ...
    fn description(&self) -> String {
        "Prepare the full REMOTE ssh->tmux shell-work path in one step. vu reuses or creates an SSH pane for the host, ensures the requested tmux session exists there, verifies tmux-native control on that same pane, and then reuses or creates a clean tmux shell target for file work. The result includes both a tmux snapshot and the selected shell target, so you can describe the tmux workspace and continue file work without attaching tmux in the visible outer pane unless the user explicitly asks to enter it.".to_string()
    }
    ...
    async fn call(&self, args: Self::Args) -> Result<Self::Output, Self::Error> {
        ...
        let reason = if shell_target_created {
            format!(
                "{} vu then created a dedicated tmux shell target `{}` for remote file work.",
                workspace.reason, shell_target.target
            )
        } else {
            format!(
                "{} vu reused the existing tmux shell target `{}` for remote file work.",
                workspace.reason, shell_target.target
            )
        };
...
mod tests {
    ...
    fn test_pane(index: usize, title: &str) -> super::PaneInfo {
        super::PaneInfo {
            ...
            address_space: PaneAddressSpace::VuPane,
    ...
    fn resolve_work_target_visible_shell_does_not_reuse_recent_local_agent_pane() {
        let (pane_tx, _pane_rx) = unbounded();
        let mut pane = test_pane(4, "vu-bench");
        ...
        pane.recent_actions.push(crate::context::PaneActionRecord {
            ...
            summary: "vu executed `cd ~/dev/temp/vu-bench && codex` in the visible shell"
                .to_string(),
            command: Some("cd ~/dev/temp/vu-bench && codex".to_string()),
        ...
        let result = resolve_work_target_candidates(
            ...
            None,
            None,
            Some("~/dev/temp/vu-bench".to_string()),
            Some("codex".to_string()),
            8,
        )
        .expect("resolve");
    ...
    fn resolve_work_target_agent_cli_does_not_reuse_continuity_only_shell_pane() {
        let (pane_tx, _pane_rx) = unbounded();
        let mut pane = test_pane(5, "vu-bench-codex");
        pane.control_capabilities = vec![PaneControlCapability::ExecVisibleShell];
        pane.visible_target.kind = PaneVisibleTargetKind::ShellPrompt;
        pane.front_state = PaneFrontState::ShellPrompt;
        pane.mode = super::PaneMode::Shell;
        ...
        pane.recent_actions.push(crate::context::PaneActionRecord {
            ...
            summary: "vu created this pane with startup command".to_string(),
            command: Some(
                "mkdir -p /Users/weyl/dev/temp/vu-bench-codex-git && cd /Users/weyl/dev/temp/vu-bench-codex-git && codex"
                    .to_string(),
            ),
            source: crate::context::PaneEvidenceSource::ActionHistory,
            confidence: crate::context::PaneConfidence::Advisory,
        ...
        let result = resolve_work_target_candidates(
            ...
            None,
            None,
            Some("/Users/weyl/dev/temp/vu-bench-codex-git".to_string()),
            Some("codex".to_string()),
            8,
        )
        .expect("resolve");
    ...
    #[test]
    fn resolve_work_target_agent_cli_reuses_vu_launched_non_shell_pane() {
        let (pane_tx, _pane_rx) = unbounded();
        let mut pane = test_pane(5, "vu-bench-codex");
        pane.is_busy = true;
        pane.recent_actions.push(crate::context::PaneActionRecord {
    ...
    fn resolve_work_target_agent_cli_reuses_dedicated_lane_despite_stale_shell_metadata() {
        let (pane_tx, _pane_rx) = unbounded();
        let mut pane = test_pane(6, "vu-bench-codex");
        pane.control_capabilities = vec![PaneControlCapability::ExecVisibleShell];
        pane.front_state = PaneFrontState::ShellPrompt;
        pane.visible_target.kind = PaneVisibleTargetKind::ShellPrompt;
        pane.recent_actions.push(crate::context::PaneActionRecord {
    ...
    fn pane_matches_cwd_uses_workspace_hint_when_live_cwd_is_stale() {
        let mut pane = test_pane(2, "vu-bench-twosum");
        pane.cwd = Some("/Users/weyl/conductor/workspaces/vu/kingston".to_string());
        pane.recent_actions.push(crate::context::PaneActionRecord {
            sequence: 1,
            kind: crate::context::PaneActionKind::PaneCreated,
            summary: "vu created this pane with startup command".to_string(),
        ...
        assert!(
            pane_matches_cwd(
                &pane,
                Some(&"/users/weyl/dev/temp/vu-bench-twosum".to_string())
            ),
    ...
    fn tmux_creation_target_prefers_requested_session() {
        let snapshot = TmuxSnapshot {
            panes: vec![
                ...
                TmuxPaneInfo {
                    session_name: "vu-bench".to_string(),
                    window_id: "@48".to_string(),
                    window_index: "1".to_string(),
                    window_name: "zsh".to_string(),
                    window_target: "vu-bench:1".to_string(),
                    pane_id: "%59".to_string(),
                    pane_index: "1".to_string(),
                    target: "vu-bench:1.1".to_string(),
                    pane_active: true,
                    window_active: true,
                    pane_current_command: Some("zsh".to_string()),
                    pane_current_path: Some("/home/w".to_string()),
        ...
        assert_eq!(
            tmux_creation_target(&snapshot, "vu-bench", TmuxExecLocation::NewWindow),
            Some("vu-bench".to_string())
        );
        assert_eq!(
            tmux_creation_target(&snapshot, "vu-bench", TmuxExecLocation::SplitHorizontal),
            Some("vu-bench:1.1".to_string())
        );
    }
    ...
    fn preferred_tmux_shell_session_defaults_to_pane_workspace_session() {
        ...
        pane.tmux_control = Some(TmuxControlState {
            session_name: Some("vu-bench".to_string()),
            mode: TmuxControlMode::Native,
            front_target: None,
            reason: "native".to_string(),
        });
