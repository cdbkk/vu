...
void *vu_sparkle_init_controller(void) {
    @try {
        ...
        NSLog(@"[vu-updater] ObjC exception during Sparkle init: %@ — %@",
...
int vu_sparkle_start_updater(void *controller) {
    @try {
        ...
        if (!ok) {
            NSLog(@"[vu-updater] startUpdater failed: %@", error);
            return 0;
        }
        return 1;
    } @catch (NSException *exception) {
        NSLog(@"[vu-updater] ObjC exception in startUpdater: %@ — %@",
              exception.name, exception.reason);
        return 0;
    }
}
...
void vu_sparkle_check_for_updates(void *controller) {
    @try {
        id ctrl = (__bridge id)controller;
        SEL checkSel = NSSelectorFromString(@"checkForUpdates:");
        ((void (*)(id, SEL, id))objc_msgSend)(ctrl, checkSel, nil);
    } @catch (NSException *exception) {
        NSLog(@"[vu-updater] ObjC exception in checkForUpdates: %@ — %@",
              exception.name, exception.reason);
    }
}
