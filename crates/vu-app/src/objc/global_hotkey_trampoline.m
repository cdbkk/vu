...
typedef void (*vu_hotkey_callback_t)(void);
...
static OSStatus vu_hotkey_handler(EventHandlerCallRef nextHandler, EventRef event, void *userData) {
    (void)nextHandler;
    (void)userData;
...
bool vu_register_global_hotkey(
    uint32_t key_code,
    bool shift,
    bool control,
    bool alt,
...
bool vu_app_is_active(void) {
    return NSApp != nil && [NSApp isActive];
}

static BOOL vu_should_cycle_window(NSEvent *event, BOOL *reverse) {
...
static NSArray<NSWindow *> *vu_cycleable_windows(void) {
...
static NSWindow *vu_find_cycleable_window(NSArray<NSWindow *> *windows, NSInteger window_number) {
...
static NSMutableArray<NSNumber *> *vu_rebuild_window_cycle_order(NSArray<NSWindow *> *windows) {
...
static NSMutableArray<NSNumber *> *vu_current_window_cycle_order(
    NSArray<NSWindow *> *windows,
    NSWindow *current,
    NSTimeInterval timestamp
) {
    BOOL expired = timestamp - g_vu_last_window_cycle_timestamp > 2.0;
    NSNumber *current_number = @(current.windowNumber);
    if (g_vu_window_cycle_order == nil ||
        expired ||
        ![g_vu_window_cycle_order containsObject:current_number]) {
        return vu_rebuild_window_cycle_order(windows);
    }

    NSMutableArray<NSNumber *> *order = [NSMutableArray arrayWithCapacity:windows.count];
    for (NSNumber *window_number in g_vu_window_cycle_order) {
        if (vu_find_cycleable_window(windows, window_number.integerValue) != nil) {
...
static void vu_cycle_app_window_at_time(BOOL reverse, NSTimeInterval timestamp) {
    NSArray<NSWindow *> *windows = vu_cycleable_windows();
    ...
    NSMutableArray<NSNumber *> *order = vu_current_window_cycle_order(windows, current, timestamp);
    NSNumber *current_number = @(current.windowNumber);
    NSUInteger current_index = [order indexOfObject:current_number];
    if (current_index == NSNotFound) {
        order = vu_rebuild_window_cycle_order(windows);
    ...
    NSWindow *target = vu_find_cycleable_window(windows, order[target_index].integerValue);
    if (target == nil) {
        return;
    }

    g_vu_window_cycle_order = order;
    g_vu_last_window_cycle_timestamp = timestamp;

    [NSApp activateIgnoringOtherApps:YES];
    [target makeKeyAndOrderFront:nil];
}

void vu_cycle_app_window(bool reverse) {
    vu_cycle_app_window_at_time(reverse, [NSDate timeIntervalSinceReferenceDate]);
}

void vu_install_window_cycle_shortcuts(void) {
    if (g_vu_window_cycle_monitor != nil) {
        return;
    }

    g_vu_window_cycle_monitor = [NSEvent addLocalMonitorForEventsMatchingMask:NSEventMaskKeyDown
                                                                       handler:^NSEvent *(NSEvent *event) {
        BOOL reverse = NO;
        if (vu_should_cycle_window(event, &reverse)) {
            vu_cycle_app_window_at_time(reverse, [NSDate timeIntervalSinceReferenceDate]);
            return nil;
        }
        return event;
