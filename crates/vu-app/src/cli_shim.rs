...
fn ensure_cli_shim_inner() -> Result<(), String> {
    ...
    let cli = macos_dir.join("vu-cli");
    if !cli.is_file() {
        return Ok(());
    }
    if !is_installed_vu_app_cli_target(&cli) {
        log::info!(
            "vu-cli shim: not linking from transient app bundle at {}",
            cli.display()
        );
        return Ok(());
    }
    ...
    match std::fs::symlink_metadata(&link) {
        Ok(metadata) if metadata.file_type().is_symlink() => {
            ...
            if !is_installed_vu_app_cli_target(&target) {
                log::info!(
                    "vu-cli shim: preserving user-managed symlink at {} -> {}",
                    link.display(),
                    target.display()
                );
                return Ok(());
        ...
        Ok(_) => {
            log::info!(
                "vu-cli shim: preserving user-managed file at {}",
                link.display()
            );
            return Ok(());
        }
...
fn is_vu_app_cli_target(target: &Path) -> bool {
    if target.file_name().and_then(|name| name.to_str()) != Some("vu-cli") {
        return false;
    }
    ...
    // repair stale links to Vu's own app bundles, but we must not replace a
    // user-managed symlink to another app that happens to ship `vu-cli`.
    matches!(
        macos_dir
            .parent()
            .and_then(Path::parent)
            .and_then(Path::file_name)
            .and_then(|name| name.to_str()),
        Some("vu.app" | "vu Beta.app" | "vu Dev.app")
    )
}

fn is_installed_vu_app_cli_target(target: &Path) -> bool {
    if !is_vu_app_cli_target(target) {
        return false;
    }
    ...
    // ~/.local/bin/vu-cli away from the installed app.
...
mod tests {
    ...
    fn app_bundle_macos_dir_is_recognized() {
        assert!(is_app_bundle_macos_dir(Path::new(
            "/Applications/vu Beta.app/Contents/MacOS"
        )));
        assert!(!is_app_bundle_macos_dir(Path::new("/tmp/target/release")));
    }

    #[test]
    fn only_vu_app_cli_symlinks_are_managed() {
        assert!(is_vu_app_cli_target(Path::new(
            "/Applications/vu Beta.app/Contents/MacOS/vu-cli"
        )));
        assert!(is_vu_app_cli_target(Path::new(
        ...
        assert!(!is_vu_app_cli_target(Path::new(
            "/Applications/Other.app/Contents/MacOS/vu-cli"
        )));
    }

    #[test]
    fn only_installed_vu_app_cli_symlinks_are_managed() {
        assert!(is_installed_vu_app_cli_target(Path::new(
            "/Applications/vu Beta.app/Contents/MacOS/vu-cli"
        )));
        assert!(is_installed_vu_app_cli_target(Path::new(
            "/Applications/vu Dev.app/Contents/MacOS/vu-cli"
        ...
        assert!(!is_installed_vu_app_cli_target(Path::new(
            "/Volumes/vu Beta/vu Beta.app/Contents/MacOS/vu-cli"
        )));
        assert!(!is_installed_vu_app_cli_target(Path::new(
            "/tmp/vu Beta.app/Contents/MacOS/vu-cli"
        )));
    }
}
