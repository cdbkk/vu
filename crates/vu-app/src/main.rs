...
fn set_dock_icon() {
    ...
    const ICON_PNG: &[u8] = include_bytes!("../../../assets/Vu-macOS-Dark-256x256@2x.png");
...
#[cfg(target_os = "linux")]
fn supports_transparent_main_window() -> bool {
    // Linux keeps Vu's integrated client-side chrome so titlebar
...
/// back to an opaque theme fill in `open_vu_window` (otherwise the
...
/// window.
///
/// Linux uses a transparent top-level surface so Vu's integrated
...
// On non-macOS targets, vu now branches by platform: Windows has the
...
fn default_window_options(config: &vu_core::Config, cx: &mut App) -> WindowOptions {
    ...
    WindowOptions {
        ...
        is_movable: false,
        #[cfg(target_os = "linux")]
        app_id: Some("co.nowledge.vu".to_owned()),
        ..Default::default()
    }
}
...
fn default_window_background(
    ...
    #[cfg(target_os = "macos")]
    {
        let effective_opacity =
            VuWorkspace::effective_terminal_opacity(config.appearance.terminal_opacity);
...
pub(crate) fn open_vu_window(
    ...
    cx.spawn(async move |cx| {
        if let Err(err) = cx.open_window(window_options, |window, cx| {
            ...
            let view = cx
                .new(|cx| VuWorkspace::from_session(config.clone(), restored_session, window, cx));
            #[cfg(target_os = "windows")]
...
fn workspace_path_error_session(path: &std::path::Path, err: &anyhow::Error) -> Session {
    ...
    let screen_text = vec![
        "Vu could not open the requested workspace profile.".to_string(),
        format!("Path: {}", path.display()),
        format!("Reason: {err}"),
        "Opened a fresh shell instead. Fix the profile and run vu <path> again.".to_string(),
        String::new(),
    ];
...
fn startup_session() -> Session {
    ...
    if live_control_endpoint_exists() {
        log::info!(
            "existing vu control endpoint detected; opening a fresh window session with shared history"
        );
...
pub(crate) fn workspace_layout_root_for_file(path: &std::path::Path) -> std::path::PathBuf {
    ...
    if parent.file_name().and_then(|name| name.to_str()) == Some(".vu")
...
fn live_control_endpoint_exists() -> bool {
    let path = vu_core::control_socket_path();
    let addr = match socket2::SockAddr::unix(&path) {
        Ok(addr) => addr,
        Err(err) => {
            log::debug!(
                "skipping vu control endpoint probe for invalid socket path {}: {err}",
                path.display()
            );
            return false;
        }
    ...
    let socket = match socket2::Socket::new(socket2::Domain::UNIX, socket2::Type::STREAM, None) {
        Ok(socket) => socket,
        Err(err) => {
            log::debug!("failed to create vu control endpoint probe socket: {err}");
            return false;
        }
    };
    ...
    match socket.connect_timeout(&addr, std::time::Duration::from_millis(75)) {
        ...
        Err(err) => {
            log::debug!(
                "vu control endpoint probe did not find a live server at {}: {err}",
                path.display()
            );
            false
        }
...
fn live_control_endpoint_exists() -> bool {
    ...
    if unsafe { WaitNamedPipeW(PCWSTR(pipe_name.as_ptr()), 75).as_bool() } {
        ...
        log::debug!(
            "vu control endpoint probe did not find a live pipe at {} within 75ms",
            path.display()
        );
        false
    }
...
pub(crate) fn toggle_global_summon(cx: &mut App) {
    ...
    if !has_windows {
        let config = vu_core::Config::load().unwrap_or_default();
        open_vu_window(config, fresh_window_session_with_history(), false, cx);
        cx.activate(true);
        return;
    }
...
pub(crate) fn app_build_number() -> String {
    ...
    let build = bundle_info_value(b"CFBundleVersion\0")
        .unwrap_or_else(|| env!("CARGO_PKG_VERSION").to_string());
    #[cfg(not(target_os = "macos"))]
    let build = option_env!("VU_RELEASE_VERSION")
        .map(str::to_owned)
        .unwrap_or_else(|| env!("CARGO_PKG_VERSION").to_string());
...
fn apply_linux_platform_override() {
    let Some(requested) = std::env::var("VU_LINUX_BACKEND")
        .ok()
    ...
    match requested.as_str() {
        "x11" => {
            if std::env::var_os("DISPLAY").is_some() {
                ...
                log::warn!(
                    "VU_LINUX_BACKEND=x11 requested, but DISPLAY is not set; keeping default compositor detection"
                );
            }
        }
        "wayland" => {
            if std::env::var_os("WAYLAND_DISPLAY").is_some() {
                ...
                log::warn!(
                    "VU_LINUX_BACKEND=wayland requested, but WAYLAND_DISPLAY is not set; keeping default compositor detection"
                );
            }
        }
        other => {
            log::warn!(
                "Ignoring unsupported VU_LINUX_BACKEND={other:?}; expected \"x11\" or \"wayland\""
            );
        }
    }
}
...
// (Sparkle lives in the same menu, and the Cocoa "About vu" gesture
...
fn about_panel_version_detail() -> String {
    ...
    let channel = vu_core::release_channel::current().display_name();

    if build.is_empty() || build == version {
        match vu_core::release_channel::current() {
            vu_core::release_channel::ReleaseChannel::Dev => "Development Build".to_string(),
            _ => channel.to_string(),
        }
    } else {
...
fn editor_binding_specs() -> Vec<BindingSpec> {
    ...
    // Editor-only editing shortcuts must not bind to Global/VuWorkspace/Input,
...
fn install_file_sidebar_shortcut_interceptor(cx: &mut App, kb: &KeybindingConfig) {
    ...
    cx.intercept_keystrokes(|event, window, cx| {
        if !event
            .context_stack
            .iter()
            .any(|context| context.contains("VuWorkspace"))
        {
            return;
        }
...
fn is_fixed_configurable_app_keybinding(key: &str, action_name: &SharedString) -> bool {
    let Some(key) = vu_core::config::canonical_keybinding(key) else {
        return false;
    };
    let fixed_next_tab = vu_core::config::canonical_keybinding("secondary-shift-]")
        .expect("fixed shortcut is valid");
    let fixed_previous_tab = vu_core::config::canonical_keybinding("secondary-shift-[")
        .expect("fixed shortcut is valid");
...
fn install_panic_hook() {
    ...
    std::panic::set_hook(Box::new(move |info| {
        ...
        let log_path = std::env::temp_dir().join("vu-panic.log");
...
mod tests {
    ...
    fn generated_specs_do_not_duplicate_same_key_action_scope() {
        ...
        for spec in &specs {
            let canonical = vu_core::config::canonical_keybinding(spec.key.as_ref())
...
fn inherit_shell_env() {
    ...
    // Skip SHLVL: the login shell increments it, and terminals spawned by vu
...
fn main() {
    ...
    if let Some(path) = std::env::var_os("VU_LOG_FILE") {
        match std::fs::File::create(&path) {
            ...
            Err(err) => {
                eprintln!(
                    "vu: failed to open VU_LOG_FILE={}: {err}",
    ...
    log::info!("vu starting (pid {})", std::process::id());
    ...
    app.run(move |cx: &mut App| {
        ...
        let channel = vu_core::release_channel::init();
        ...
        // Load and activate vu's design theme (synced to terminal theme)
        theme::init_theme(
            cx,
        ...
        open_vu_window(config.clone(), startup_session(), true, cx);
