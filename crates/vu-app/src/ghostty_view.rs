...
unsafe extern "C" {
    ...
    fn vu_ghostty_surface_install_backing_observer(view: *mut c_void, surface: *mut c_void);
    fn vu_ghostty_surface_remove_backing_observer(view: *mut c_void);
    fn vu_ghostty_surface_sync_backing(
        view: *mut c_void,
        surface: *mut c_void,
        logical_width: f64,
        logical_height: f64,
...
impl GhosttyView {
    ...
    fn detach_host_view(&mut self) {
        ...
        if let Some(nsview) = self.nsview {
            unsafe {
                vu_ghostty_surface_remove_backing_observer(nsview as *mut c_void);
            }
        }
    ...
    fn ensure_initialized(
        ...
        match self.app.new_surface(
            ...
            Ok(terminal) => {
                ...
                self.next_surface_init_retry_at = None;
                unsafe {
                    vu_ghostty_surface_install_backing_observer(
                        nsview as *mut c_void,
    ...
    fn update_frame(&mut self, bounds: Bounds<Pixels>, window: &Window) {
        // `last_bounds` is Vu's layout cache, not Ghostty's protocol state.
    ...
    fn sync_native_scroll_view(&mut self) {
        ...
        unsafe {
            ...
            // mutation through a local tuple cache. Vu drives this AppKit
            ...
            // frame must be deterministically reapplied from Vu-owned pane
