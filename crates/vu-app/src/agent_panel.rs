...
use vu_core::harness::HarnessEvent;
...
impl PanelState {
    ...
    pub fn from_conversation(conv: &vu_agent::Conversation) -> Self {
        let mut state = Self::new();
        for msg in &conv.messages {
            let role = match msg.role {
                vu_agent::MessageRole::User => "user",
                vu_agent::MessageRole::Assistant => "assistant",
                vu_agent::MessageRole::System => "system",
                vu_agent::MessageRole::Tool => "system",
            };
...
impl AgentPanel {
    ...
    pub fn toggle_history(&mut self, cx: &mut Context<Self>) {
        ...
        if self.showing_history {
            self.conversation_list = vu_agent::Conversation::list_all();
        }
        self.content_reveal
    ...
    pub fn complete_response(&mut self, msg: &vu_agent::Message, cx: &mut Context<Self>) {
        let old_count = self.state.messages.len();
        self.state
...
mod tests {
    ...
    use vu_agent::{AgentConfig, ProviderConfig, ProviderKind};
