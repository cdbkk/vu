//! Linux terminal view backed by vu's local Unix PTY + libghostty-vt
//! parser. Phase 4 styled-cell renderer: this view consumes the parsed
//! `ScreenSnapshot` from `vu-ghostty` and paints each row as a GPUI
...
/// per `vu-ghostty/src/vt.rs::read_cell`) into a GPUI `Hsla`.
