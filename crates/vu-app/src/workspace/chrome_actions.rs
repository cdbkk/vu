...
impl VuWorkspace {
    ...
    pub(super) fn export_workspace_layout(
        ...
        let export_dir = root.join(".vu");
        if let Err(err) = std::fs::create_dir_all(&export_dir) {
            Self::show_layout_profile_error(window, cx, "Could not prepare .vu directory", err);
            return;
        }
    ...
    pub(super) fn prompt_for_workspace_layout_path(
        ...
        let paths = cx.prompt_for_paths(PathPromptOptions {
            ...
            directories: true,
            multiple: false,
            prompt: Some("Choose a project folder or Vu workspace layout".into()),
        });
                        ...
                        if open_in_new_window {
                            match crate::session_from_workspace_layout_path(&path) {
                                Ok(session) => {
                                    ...
                                    crate::open_vu_window(config, session, false, cx);
                                }
