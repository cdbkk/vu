...
fn surface_key_bytes_preserves_literal_character_case() {
    assert_eq!(VuWorkspace::surface_key_bytes("A").unwrap(), b"A");
    assert_eq!(VuWorkspace::surface_key_bytes("z").unwrap(), b"z");
}

#[test]
fn surface_key_bytes_matches_named_keys_case_insensitively() {
    assert_eq!(VuWorkspace::surface_key_bytes("ENTER").unwrap(), b"\n");
    assert_eq!(
        VuWorkspace::surface_key_bytes("Ctrl-C").unwrap(),
        vec![0x03]
    );
    assert_eq!(
        VuWorkspace::surface_key_bytes("Ctrl-]").unwrap(),
        vec![0x1d]
    );
    assert_eq!(
        VuWorkspace::surface_key_bytes("control-\\").unwrap(),
        vec![0x1c]
    );
    assert_eq!(VuWorkspace::surface_key_bytes("C-/").unwrap(), vec![0x1f]);
    assert_eq!(
        VuWorkspace::surface_key_bytes("ctrl-2").unwrap(),
        vec![0x00]
    );
    assert_eq!(VuWorkspace::surface_key_bytes("C-?").unwrap(), vec![0x7f]);
}

#[test]
fn new_tab_requires_immediate_ui_sync() {
    let policy = VuWorkspace::new_tab_sync_policy_for_tests();
...
fn promoting_single_tab_to_tab_strip_requires_deferred_top_chrome_refresh() {
    assert!(VuWorkspace::should_defer_top_chrome_refresh_when_tab_strip_appears_for_tests());
}
