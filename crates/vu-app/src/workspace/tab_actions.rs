...
impl VuWorkspace {
    ...
    pub(crate) fn on_terminal_split_requested(
        ...
        let (direction, placement) = match event.0 {
            vu_ghostty::GhosttySplitDirection::Right => {
                (SplitDirection::Horizontal, SplitPlacement::After)
            }
            vu_ghostty::GhosttySplitDirection::Down => {
                (SplitDirection::Vertical, SplitPlacement::After)
            }
            vu_ghostty::GhosttySplitDirection::Left => {
                (SplitDirection::Horizontal, SplitPlacement::Before)
            }
            vu_ghostty::GhosttySplitDirection::Up => {
    ...
    pub(super) fn set_tab_color(
        &mut self,
        index: usize,
        color: Option<vu_core::session::TabAccentColor>,
        cx: &mut Context<Self>,
    ) {
        if index >= self.tabs.len() {
            return;
