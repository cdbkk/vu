//! Windows terminal view — drives the `vu-ghostty` Windows backend
...
//! `vu-app` (terminal_pane.rs, workspace/) compiles unchanged. The
...
use vu_ghostty::windows::host_view::{MouseEventMods, RenderSession};
use vu_ghostty::windows::render::{FrameBgra, RenderOutcome};
...
impl GhosttyView {
    ...
    fn forward_scroll(&self, pos: Point<Pixels>, delta: ScrollDelta, mods: MouseEventMods) -> bool {
        ...
        // mouse tracking (SGR). Otherwise scroll Vu's own viewport via
        // libghostty-vt. Shift bypasses reporting per xterm convention
        // so the user can scroll Vu's scrollback even when a TUI has
        // `set mouse=a`.
