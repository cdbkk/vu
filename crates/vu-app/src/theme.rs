...
const VU_SHELL_HIGHLIGHTS: &str = r####"
[
  (string)
  (raw_string)
  (heredoc_body)
...
/// Initialize the vu theme system.
///
...
fn register_command_prompt_language() {
    ...
    if registry.language("vu-shell").is_some() {
        return;
    }
    ...
    bash.name = "vu-shell".into();
    bash.highlights = VU_SHELL_HIGHLIGHTS.into();
    registry.register("vu-shell", &bash);
}
...
/// settings UI shows and what `vu-core::config::default_font_family`
...
/// Apply vu's scrollbar overrides after any Theme::change call.
...
fn apply_gpui_theme_by_name(terminal_theme_name: &str, cx: &mut App) {
    let (dark_name, light_name) = match terminal_theme_name {
        "catppuccin-mocha" => ("Catppuccin Mocha", "Vu Light"),
        "tokyonight" => ("Tokyo Night", "Vu Light"),
        _ => ("Vu Dark", "Vu Light"),
    };
...
fn generate_gpui_theme_json(tt: &TerminalTheme) -> String {
    ...
    let theme_name = format!("vu-gen-{}", tt.name);
