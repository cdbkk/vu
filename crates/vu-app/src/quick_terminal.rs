...
fn capture_return_pid() -> Option<i32> {
    ...
    let frontmost_pid = unsafe { vu_quick_terminal_frontmost_app_pid() };
...
/// already made vu frontmost. The recorded app may be vu itself: the return
/// target is "whatever app was active before Quick Terminal", not "non-vu".
pub fn remember_active_app_pid(pid: i32) {
    let current_pid = std::process::id() as i32;
    if let Some(pid) = valid_pid(pid) {
        QUICK_TERMINAL_STATE.with(|state| {
            let mut state = state.borrow_mut();
            // Opening/showing Quick Terminal activates vu as the owner app.
            // Do not let that synthetic activation overwrite the app that was
            // active before Quick Terminal. A vu pid observed while the quick
...
/// callback, before macOS activates vu as the hotkey owner. Call this
...
unsafe extern "C" {
    fn vu_quick_terminal_configure(window_ptr: *mut std::ffi::c_void);
    fn vu_quick_terminal_init();
    fn vu_quick_terminal_prepare_destroy(window_ptr: *mut std::ffi::c_void);
    fn vu_quick_terminal_slide_in(window_ptr: *mut std::ffi::c_void);
    fn vu_quick_terminal_slide_out(window_ptr: *mut std::ffi::c_void, return_pid: i32);
    fn vu_quick_terminal_window_from_view(
        view_ptr: *mut std::ffi::c_void,
    ) -> *mut std::ffi::c_void;
    fn vu_quick_terminal_active_display_visible_frame(
        x: *mut f64,
        y: *mut f64,
        width: *mut f64,
        height: *mut f64,
    ) -> bool;
    fn vu_quick_terminal_frontmost_app_pid() -> i32;
    fn vu_quick_terminal_is_main_thread() -> bool;
}
...
pub fn store_window_ptr(window_ptr: *mut std::ffi::c_void) {
    ...
    unsafe { vu_quick_terminal_configure(window_ptr) };
    unsafe { vu_quick_terminal_slide_in(window_ptr) };
}

pub fn window_from_view_ptr(view_ptr: *mut std::ffi::c_void) -> Option<*mut std::ffi::c_void> {
    let window_ptr = unsafe { vu_quick_terminal_window_from_view(view_ptr) };
    (!window_ptr.is_null()).then_some(window_ptr)
}

/// Toggle called from the hotkey handler with a pid captured *before*
/// macOS activates vu as the hotkey owner. This is the preferred entry
...
/// Captures the frontmost pid at call time — may already be vu itself
    ...
    if let Some(window_ptr) = window_ptr {
        QUICK_TERMINAL_STATE.with(|state| {
            ...
            if state.visible {
                let return_pid = state.return_pid.take();
                unsafe {
                    vu_quick_terminal_slide_out(raw, return_pid.unwrap_or(0));
                }
                state.visible = false;
            } else {
                // Use the pre-captured pid (taken before vu was activated).
                state.return_pid = pre_captured_pid;
                unsafe {
                    vu_quick_terminal_slide_in(raw);
                }
                state.visible = true;
            }
        });
...
pub fn active_display_visible_bounds() -> Option<Bounds<Pixels>> {
    ...
    let mut height = 0.0;
    let ok = unsafe {
        vu_quick_terminal_active_display_visible_frame(&mut x, &mut y, &mut width, &mut height)
    };
...
/// focus returns to the previously active app, not the vu main window.
pub fn force_hide() {
    QUICK_TERMINAL_STATE.with(|state| {
        let mut state = state.borrow_mut();
...
pub fn hide() {
    QUICK_TERMINAL_STATE.with(|state| {
        ...
        if let Some(window_ptr) = state.raw_ptr {
            unsafe {
                vu_quick_terminal_slide_out(window_ptr as *mut std::ffi::c_void, 0);
            }
        }
    });
}
...
#[unsafe(no_mangle)]
extern "C" fn vu_quick_terminal_remember_active_app(pid: i32) {
...
#[unsafe(no_mangle)]
extern "C" fn vu_quick_terminal_handle_resign_key() {
    debug_assert!(unsafe { vu_quick_terminal_is_main_thread() });
    hide();
}
...
pub fn reset_destroyed_window() {
    ...
    if let Some(window_ptr) = window_ptr {
        unsafe {
            vu_quick_terminal_prepare_destroy(window_ptr as *mut std::ffi::c_void);
        }
    }
}
...
mod tests {
    ...
    fn quick_terminal_binding_default_is_non_empty() {
        assert!(
            !vu_core::config::KeybindingConfig::default()
                .quick_terminal
                .trim()
                .is_empty()
        );
