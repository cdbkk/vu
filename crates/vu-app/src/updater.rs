...
#[cfg(target_os = "macos")]
unsafe extern "C" {
    fn vu_sparkle_init_controller() -> *mut std::ffi::c_void;
    fn vu_sparkle_check_for_updates(controller: *mut std::ffi::c_void);
}
...
fn init_inner() -> bool {
    ...
    unsafe {
        ...
        let controller = vu_sparkle_init_controller();
...
pub fn check_for_updates() {
    ...
    unsafe {
        vu_sparkle_check_for_updates(controller);
    }
}
...
/// installer can replace `vu-app.exe`. Windows only.
///
...
pub fn apply_update_in_place() {
    ...
    const INSTALL_URL: &str = "https://vu-releases.nowledge.co/install.ps1";
        ...
        .spawn()
    {
        Ok(_) => {
            ...
            // vu-app` as a belt-and-braces step, but exiting cleanly
...
/// cleanly so the script can replace `~/.local/bin/vu` and any
...
/// binary into place. Move-then-overwrite means the running vu
/// inode survives — the kernel keeps the old text in memory until
/// this process exits — so re-running vu after the installer
/// finishes picks up the new build. We `setsid -f` to detach so
/// killing vu does not kill the installer.
///
...
pub fn apply_update_in_place() {
    ...
    const DEFAULT_INSTALL_URL: &str = "https://vu-releases.nowledge.co/install.sh";

    // `VU_INSTALL_URL` overrides the script source for offline /
    // local-server verification. Same env-only opt-in as
    // `VU_APPCAST_BASE` — release builds default to the public
    // gh-pages-served install.sh; tests can point at their own.
    let install_url = std::env::var("VU_INSTALL_URL")
        .ok()
    ...
    //      `vu` exits as if the install kicked off, and they're
    ...
    //
    // `export VU_INSTALL_VERSION=...` (rather than the
    ...
    // it survives `vu`'s exit; the trailing `>/dev/null 2>&1
    ...
    // so user-supplied env (`VU_INSTALL_URL`) and appcast-supplied
    ...
    let pipeline = match target_version.as_deref() {
        Some(version) => format!(
            "export VU_INSTALL_VERSION={version}; \
...
/// `https://vu-releases.nowledge.co/appcast/{channel}-{platform}-{arch}.xml`,
...
mod notify_impl {
    ...
    use vu_core::release_channel::{self, ReleaseChannel};
            ...
            <rss xmlns:sparkle="http://www.andymatuschak.org/xml-namespaces/sparkle">
              <channel>
                <item>
                  ...
                  <enclosure url="https://example.com/vu-0.8.0.zip" length="12345" type="application/zip" sparkle:edSignature="abc"/>
                </item>
              </channel>
            </rss>"#;
            let (v, u) = parse_latest(xml).unwrap();
            assert_eq!(v, "0.8.0");
            assert_eq!(u, "https://example.com/vu-0.8.0.zip");
        }
