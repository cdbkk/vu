...
impl ReleaseChannel {
    ...
    ///
    /// `VU_APPCAST_BASE` overrides the host + path prefix at
    ...
    pub fn feed_url(self, platform: &str, arch: &str) -> String {
        ...
        // `VU_APPCAST_BASE=" http://localhost:8765/appcast/ "`
        ...
        let base = std::env::var("VU_APPCAST_BASE")
            .ok()
            .map(|value| value.trim().trim_end_matches('/').to_string())
            .filter(|value| !value.is_empty())
            .unwrap_or_else(|| "https://vu-releases.nowledge.co/appcast".to_string());
        format!(
    ...
    /// Parse from the `VuReleaseChannel` value baked into Info.plist.
...
/// Read `VuReleaseChannel` from the main bundle's Info.plist.
#[cfg(target_os = "macos")]
fn detect_channel() -> ReleaseChannel {
    ...
    unsafe {
        ...
        let key: *mut objc::runtime::Object =
            msg_send![class!(NSString), stringWithUTF8String: b"VuReleaseChannel\0".as_ptr()];
...
fn detect_channel() -> ReleaseChannel {
    ...
    // exports `VU_RELEASE_CHANNEL=beta|stable` before `cargo build` so
    // `option_env!` captures it in the binary. Fall back to the runtime
    // env var for local overrides, and finally to Dev.
    if let Some(baked) = option_env!("VU_RELEASE_CHANNEL") {
        return ReleaseChannel::from_str(baked);
    }
    match std::env::var("VU_RELEASE_CHANNEL").as_deref() {
