...
pub const WORKSPACE_ERROR_SURFACE_OWNER: &str = "vu:workspace-error";
...
impl Session {
    ...
    pub fn session_path() -> PathBuf {
        if let Some(path) = std::env::var_os("VU_SESSION_PATH") {
            return PathBuf::from(path);
        }
        vu_paths::app_data_dir().join("session.json")
    }
}
...
impl GlobalHistoryState {
    ...
    pub fn history_path() -> PathBuf {
        if let Some(path) = std::env::var_os("VU_HISTORY_PATH") {
            return PathBuf::from(path);
        }
        vu_paths::app_data_dir().join("history.json")
    }
}
