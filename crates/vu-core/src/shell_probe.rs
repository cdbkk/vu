...
const PREFIX: &str = "__VU_SHELL_PROBE__";
...
pub fn build_shell_probe_command(nonce: &str) -> String {
    format!(
        r##"sh -c '
vu_probe_emit() {{ printf "__VU_SHELL_PROBE__\t%s\t%s\n" "$1" "$2"; }}
printf "%s\n" "__VU_SHELL_PROBE_BEGIN_{nonce}__"
vu_probe_emit host "$(hostname 2>/dev/null || uname -n 2>/dev/null || printf "")"
vu_probe_emit pwd "$(pwd 2>/dev/null || printf "")"
vu_probe_emit term "${{TERM-}}"
vu_probe_emit term_program "${{TERM_PROGRAM-}}"
vu_probe_emit ssh_connection "${{SSH_CONNECTION-}}"
vu_probe_emit ssh_tty "${{SSH_TTY-}}"
vu_probe_emit tmux_env "${{TMUX-}}"
vu_probe_emit nvim_listen_address "${{NVIM_LISTEN_ADDRESS-}}"
if [ -n "${{TMUX-}}" ] && command -v tmux >/dev/null 2>&1; then
  vu_probe_emit tmux_available "1"
  vu_probe_emit tmux_session_name "$(tmux display-message -p "#{{session_name}}" 2>/dev/null || printf "")"
  vu_probe_emit tmux_window_id "$(tmux display-message -p "#{{window_id}}" 2>/dev/null || printf "")"
  vu_probe_emit tmux_window_name "$(tmux display-message -p "#{{window_name}}" 2>/dev/null || printf "")"
  vu_probe_emit tmux_pane_id "$(tmux display-message -p "#{{pane_id}}" 2>/dev/null || printf "")"
  vu_probe_emit tmux_pane_current_command "$(tmux display-message -p "#{{pane_current_command}}" 2>/dev/null || printf "")"
  vu_probe_emit tmux_pane_current_path "$(tmux display-message -p "#{{pane_current_path}}" 2>/dev/null || printf "")"
  vu_probe_emit tmux_client_tty "$(tmux display-message -p "#{{client_tty}}" 2>/dev/null || printf "")"
else
  vu_probe_emit tmux_available "0"
fi
printf "%s\n" "__VU_SHELL_PROBE_END_{nonce}__"
'"##,
    )
}

pub fn parse_shell_probe_lines(lines: &[String], nonce: &str) -> Result<ShellProbeResult, String> {
    let begin = format!("__VU_SHELL_PROBE_BEGIN_{nonce}__");
    let end = format!("__VU_SHELL_PROBE_END_{nonce}__");
...
mod tests {
    ...
    fn build_command_uses_nonce_markers() {
        ...
        assert!(cmd.contains("__VU_SHELL_PROBE_BEGIN_abc123__"));
        assert!(cmd.contains("__VU_SHELL_PROBE_END_abc123__"));
    ...
    fn parse_probe_block_extracts_tmux_context() {
        let lines = vec![
            ...
            "__VU_SHELL_PROBE_BEGIN_42__".to_string(),
            "__VU_SHELL_PROBE__\thost\thaswell".to_string(),
            "__VU_SHELL_PROBE__\tpwd\t/home/weyl/project".to_string(),
            "__VU_SHELL_PROBE__\tssh_connection\t1.2.3.4 1111 5.6.7.8 22".to_string(),
            "__VU_SHELL_PROBE__\ttmux_env\t/tmp/tmux-1000/default,123,0".to_string(),
            "__VU_SHELL_PROBE__\ttmux_available\t1".to_string(),
            "__VU_SHELL_PROBE__\ttmux_session_name\twork".to_string(),
            "__VU_SHELL_PROBE__\ttmux_window_id\t@3".to_string(),
            "__VU_SHELL_PROBE__\ttmux_pane_id\t%17".to_string(),
            "__VU_SHELL_PROBE__\ttmux_pane_current_command\tnvim".to_string(),
            "__VU_SHELL_PROBE_END_42__".to_string(),
        ];
