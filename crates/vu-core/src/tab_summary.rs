...
async fn request_summary(
    ...
    let recent_cmds = if req.recent_commands.is_empty() {
        "(none captured by Vu — user may have typed directly)".to_string()
    } else {
        req.recent_commands
            .iter()
            .take(3)
    ...
    let prompt = format!(
        ...
        recent_cmds = if recent_cmds == "(none captured by Vu — user may have typed directly)" {
            "(none)".to_string()
        } else {
            recent_cmds
        },
...
mod tests {
    ...
    /// it next to other vu-core logic is fine.
