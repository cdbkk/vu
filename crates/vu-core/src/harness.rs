...
struct RequestTrace {
    steps: Vec<vu_agent::conversation::AgentStep>,
    thinking: String,
}
...
pub enum HarnessEvent {
    ...
    Step(vu_agent::conversation::AgentStep),
...
impl AgentHarness {
    ...
    pub fn build_context_from_snapshot(
        ...
        focused_observation: &vu_agent::context::PaneObservationFrame,
        focused_runtime: &vu_agent::context::PaneRuntimeState,
        other_panes: Vec<vu_agent::context::PaneSummary>,
    ) -> TerminalContext {
        let cwd = focused_observation.cwd.clone();

        let ssh_host = focused_runtime.remote_host.clone();
        let focused_control = vu_agent::control::PaneControlState::from_runtime(focused_runtime);
        let focused_remote_workspace =
            vu_agent::context::remote_workspace_anchor(focused_runtime, focused_observation);
    ...
    pub fn send_message(
        ...
        agent_config: vu_agent::AgentConfig,
        content: String,
        ...
        // ToolApprovalNeeded events, the receiver goes to the VuHook.
                        ...
                        let mapped = match event {
                            ...
                            AgentEvent::ToolCallStart {
                                ...
                                trace_for_bridge.lock().steps.push(
                                    vu_agent::conversation::AgentStep::ToolCall {
                            ...
                            AgentEvent::ToolCallComplete {
                                ...
                                trace_for_bridge.lock().steps.push(
                                    vu_agent::conversation::AgentStep::ToolResult {
                                        call_id: Some(call_id.clone()),
                                        tool: tool_name.clone(),
                                        output: result.clone(),
                                        success: true,
    ...
    pub fn config(&self) -> &vu_agent::AgentConfig {
        &self.config
    }

    pub fn update_config(&mut self, config: vu_agent::AgentConfig) {
        self.config = config;
    }
    ...
    pub fn active_model_name_for(config: &vu_agent::AgentConfig) -> String {
...
mod tests {
    ...
    impl TempDir {
        fn new(name: &str) -> Self {
            ...
            let path = std::env::temp_dir()
                .join(format!("vu-harness-{name}-{}-{id}", std::process::id()));
    ...
    fn harness_with_skill_paths(global: &Path) -> AgentHarness {
        ...
        config.skills = SkillsConfig {
            project_paths: vec![".vu/skills".into()],
    ...
    fn async_skill_scan_reloads_when_project_skill_root_appears() {
        ...
        let project_skills = cwd_b.join(".vu/skills");
    ...
    fn async_skill_scan_ignores_stale_results() {
        ...
        fs::create_dir_all(&cwd_b).unwrap();
        write_skill(&global, "global-skill");
        write_skill(&cwd_a.join(".vu/skills"), "a-skill");
        write_skill(&cwd_b.join(".vu/skills"), "b-skill");
    ...
    fn duplicate_in_flight_scan_can_become_latest_request_again() {
        ...
        write_skill(&cwd_a.join(".vu/skills"), "new-a-skill");
...
async fn auto_query_focused_tmux_snapshot(
    ...
    if pane_tx
        .send(PaneRequest {
            query: vu_agent::PaneQuery::TmuxList {
                pane: vu_agent::tools::PaneSelector::new(Some(pane_index), Some(pane_id)),
            },
            response_tx,
        })
        .is_err()
    ...
    match response {
        vu_agent::PaneResponse::TmuxList(snapshot) => {
        ...
        vu_agent::PaneResponse::Error(err) => {
            log::debug!(
