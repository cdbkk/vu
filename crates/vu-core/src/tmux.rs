...
fn list_field_separator(nonce: &str) -> String {
    format!("__VU_TMUX_FIELD_{nonce}__")
}

pub fn build_tmux_list_command(nonce: &str) -> String {
    let sep = list_field_separator(nonce);
    let pane_format = format!(
        "__VU_TMUX__{sep}#{{session_name}}{sep}#{{window_id}}{sep}#{{window_index}}{sep}#{{window_name}}{sep}#{{pane_id}}{sep}#{{pane_index}}{sep}#{{pane_active}}{sep}#{{window_active}}{sep}#{{pane_current_command}}{sep}#{{pane_current_path}}"
    );
    shell_wrap_script(format!(
        r#"
printf "%s\n" "__VU_TMUX_BEGIN_{nonce}__"
tmux list-panes -a -F {} 2>/dev/null
printf "%s\n" "__VU_TMUX_END_{nonce}__"
        "#,
...
pub fn parse_tmux_list_lines(lines: &[String], nonce: &str) -> Result<TmuxSnapshot, String> {
    let begin = format!("__VU_TMUX_BEGIN_{nonce}__");
    let end = format!("__VU_TMUX_END_{nonce}__");
...
pub fn build_tmux_capture_command(nonce: &str, target: Option<&str>, lines: usize) -> String {
    ...
    shell_wrap_script(format!(
        r#"
printf "%s\n" "__VU_TMUX_CAPTURE_BEGIN_{nonce}__"
tmux capture-pane -p -J {target_flag}-S -{lines} 2>/dev/null
printf "%s\n" "__VU_TMUX_CAPTURE_END_{nonce}__"
        "#
    ))
}
...
pub fn parse_tmux_capture_lines(
    ...
    let begin = format!("__VU_TMUX_CAPTURE_BEGIN_{nonce}__");
    let end = format!("__VU_TMUX_CAPTURE_END_{nonce}__");
...
pub fn build_tmux_exec_command(
    ...
    let mut args = vec![
        ...
        "-F".to_string(),
        shell_quote(&format!(
            "__VU_TMUX_EXEC__{sep}#{{session_name}}{sep}#{{window_id}}{sep}#{{window_index}}{sep}#{{window_name}}{sep}#{{pane_id}}{sep}#{{pane_index}}",
    ...
    shell_wrap_script(format!(
        r#"
printf "%s\n" "__VU_TMUX_EXEC_BEGIN_{nonce}__"
{}
printf "%s\n" "__VU_TMUX_EXEC_END_{nonce}__"
        "#,
        args.join(" ")
    ))
}
...
pub fn parse_tmux_exec_lines(
    ...
    let begin = format!("__VU_TMUX_EXEC_BEGIN_{nonce}__");
    let end = format!("__VU_TMUX_EXEC_END_{nonce}__");
    ...
    for line in &lines[begin_idx + 1..end_idx] {
        let trimmed = line.trim_end_matches('\r');
        if !trimmed.starts_with("__VU_TMUX_EXEC__") {
            continue;
        }
...
mod tests {
    ...
    fn list_parser_extracts_tmux_panes() {
        let lines = vec![
            "__VU_TMUX_BEGIN_x__".to_string(),
            "__VU_TMUX__\twork\t@1\t1\tshell\t%3\t0\t1\t1\tzsh\t/home/w".to_string(),
            "__VU_TMUX__\twork\t@2\t2\tcodex\t%7\t1\t0\t0\tcodex\t/home/w/repo".to_string(),
            "__VU_TMUX_END_x__".to_string(),
        ];
    ...
    fn list_parser_extracts_nonce_scoped_separator_rows() {
        let lines = vec![
            "__VU_TMUX_BEGIN_x__".to_string(),
            "__VU_TMUX____VU_TMUX_FIELD_x__work__VU_TMUX_FIELD_x__@1__VU_TMUX_FIELD_x__1__VU_TMUX_FIELD_x__shell__VU_TMUX_FIELD_x__%3__VU_TMUX_FIELD_x__0__VU_TMUX_FIELD_x__1__VU_TMUX_FIELD_x__1__VU_TMUX_FIELD_x__zsh__VU_TMUX_FIELD_x__/home/w".to_string(),
            "__VU_TMUX_END_x__".to_string(),
        ];
        let snapshot = parse_tmux_list_lines(&lines, "x").expect("parse");
    ...
    fn capture_parser_extracts_content() {
        let lines = vec![
            "prompt".to_string(),
            "__VU_TMUX_CAPTURE_BEGIN_x__".to_string(),
            "hello".to_string(),
            "world".to_string(),
            "__VU_TMUX_CAPTURE_END_x__".to_string(),
        ];
    ...
    fn builders_include_markers() {
        ...
        assert!(
            ...
            .contains("__VU_TMUX_EXEC_BEGIN_n__")
        );
    ...
    fn exec_parser_extracts_spawned_target() {
        let lines = vec![
            "__VU_TMUX_EXEC_BEGIN_x__".to_string(),
            "__VU_TMUX_EXEC____VU_TMUX_FIELD_x__work__VU_TMUX_FIELD_x__@9__VU_TMUX_FIELD_x__9__VU_TMUX_FIELD_x__monitor__VU_TMUX_FIELD_x__%31__VU_TMUX_FIELD_x__0".to_string(),
            "__VU_TMUX_EXEC_END_x__".to_string(),
        ];
        let exec =
    ...
    fn exec_parser_accepts_legacy_tab_rows() {
        let lines = vec![
            "__VU_TMUX_EXEC_BEGIN_x__".to_string(),
            "__VU_TMUX_EXEC__\twork\t@9\t9\tmonitor\t%31\t0".to_string(),
            "__VU_TMUX_EXEC_END_x__".to_string(),
        ];
        let exec =
            parse_tmux_exec_lines(&lines, "x", TmuxExecLocation::NewWindow, true).expect("exec");
