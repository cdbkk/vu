    ...
    pub fn from_runtime(runtime: &PaneRuntimeState) -> Self {
        ...
        if runtime.shell_context_fresh {
            ...
            if runtime.screen_tmux_like {
                notes.push(format!(
                    "The current screen still looks like a prompt inside tmux session `{session}`, and recent vu-caused tmux setup keeps a durable same-session tmux shell anchor alive."
                ));
            } else {
                notes.push(format!(
                    "The current screen still looks like a prompt on the shell that prepared tmux session `{session}`, and recent vu-caused tmux setup keeps a durable same-session tmux control anchor alive."
                ));
            }
            notes.push(
            ...
            let session = tmux_session_hint(runtime).unwrap_or_else(|| "tmux".to_string());
            notes.push(format!(
                "A recent vu-executed tmux command targeted session `{session}` from the current fresh shell prompt."
            ));
            notes.push(
        ...
        if let Some(action) = runtime.recent_actions.last() {
            notes.push(format!("Recent vu action: {}.", action.summary));
        }
        ...
        if let Some(session_name) = tmux_session_hint(runtime) {
                    ...
                    .is_some()
            {
                (
                    ...
                    "A fresh shell probe confirmed a same-session tmux shell anchor. vu can query tmux state, create tmux targets, and send tmux-native keys through that shell."
                        .to_string(),
                )
            } else if runtime.screen_prompt_like && runtime.screen_tmux_like {
                (
                    PaneAttachmentAuthority::ActionHistory,
                    "A recent vu-executed tmux command and the current prompt-like tmux screen keep a durable same-session tmux control anchor alive."
                        .to_string(),
                )
            } else {
                (
                    PaneAttachmentAuthority::ActionHistory,
                    "A recent vu-executed tmux command and the current prompt-like shell keep a durable same-session tmux control anchor alive."
                        .to_string(),
                )
            };
            attachments.push(PaneProtocolAttachment {
    ...
    Some(TmuxControlState {
        ...
        reason: if has_current_tmux
            ...
            && runtime
                ...
                .is_some()
        {
            "vu has a typed same-session tmux shell anchor for this pane. Native tmux query, capture, run-command, and send-keys are available through that anchor.".to_string()
        } else if has_native_tmux_anchor && runtime.screen_prompt_like {
            if runtime.screen_tmux_like {
                "The current screen still looks like a prompt inside tmux, and recent Vu-caused tmux setup from this pane keeps a durable same-session tmux shell anchor alive. Native tmux query, capture, run-command, and send-keys remain available through that prompt-like tmux shell.".to_string()
            } else {
                "The current screen still looks like a prompt on the shell that prepared tmux, and recent Vu-caused tmux setup from this pane keeps a durable same-session tmux control anchor alive. Native tmux query, capture, run-command, and send-keys remain available through that prompt-like shell.".to_string()
            }
        } else if has_native_tmux_anchor {
            "A recent vu-executed tmux command and the current fresh shell prompt give vu a native tmux control anchor for this session, even though tmux is not yet the proven front-most target.".to_string()
        } else if has_current_tmux {
            "vu can identify a current tmux layer in this pane, but it does not yet have a same-session tmux control channel. Native tmux pane/window targeting and tmux command execution are unavailable.".to_string()
        } else {
    ...
    fn fresh_shell_with_recent_tmux_command_gets_native_tmux_anchor() {
        let runtime = PaneRuntimeState {
            ...
            recent_actions: vec![PaneActionRecord {
                ...
                summary: "vu executed `tmux has-session -t vu-bench 2>/dev/null || tmux new-session -d -s vu-bench` in the visible shell".to_string(),
                command: Some(
                    "tmux has-session -t vu-bench 2>/dev/null || tmux new-session -d -s vu-bench"
                        .to_string(),
                ),
