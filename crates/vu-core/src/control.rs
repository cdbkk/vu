...
///
/// Debug builds intentionally use a distinct endpoint so `cargo run -p vu`
/// can coexist with an installed production/beta Vu without making startup
/// believe the dev process is a second window for the installed app. The
/// default can be overridden with the `VU_SOCKET_PATH` env var on every
/// platform.
#[cfg(all(unix, not(debug_assertions)))]
pub const DEFAULT_SOCKET_PATH: &str = "/tmp/vu.sock";
#[cfg(all(unix, debug_assertions))]
pub const DEFAULT_SOCKET_PATH: &str = "/tmp/vu-debug.sock";
#[cfg(all(windows, not(debug_assertions)))]
pub const DEFAULT_SOCKET_PATH: &str = r"\\.\pipe\vu";
#[cfg(all(windows, debug_assertions))]
pub const DEFAULT_SOCKET_PATH: &str = r"\\.\pipe\vu-debug";
#[cfg(all(not(any(unix, windows)), not(debug_assertions)))]
pub const DEFAULT_SOCKET_PATH: &str = "vu.sock";
#[cfg(all(not(any(unix, windows)), debug_assertions))]
pub const DEFAULT_SOCKET_PATH: &str = "vu-debug.sock";
...
pub fn spawn_control_socket_server(
    ...
    anyhow::bail!(
        "vu control socket is not supported on this target — \
...
async fn handle_json_rpc_request(
    ...
    match response_rx.await {
        ...
        Err(_) => JsonRpcResponse {
            ...
            id,
            result: None,
            error: Some(ControlError::internal("vu control request dropped").into_json_rpc()),
        },
    }
}
...
fn prepare_socket_path(path: &Path) -> Result<()> {
    ...
    if path.exists() {
        match std::os::unix::net::UnixStream::connect(path) {
            Ok(_) => {
                anyhow::bail!(
                    "another vu control socket is already listening at {}",
                    path.display()
                );
            }
            Err(_) => {
...
mod tests {
    ...
    fn unknown_method_returns_json_rpc_error() {
        let err = ControlCommand::from_rpc("vu.unknown", json!({})).expect_err("should fail");
