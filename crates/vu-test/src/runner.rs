/// Test runner — launches a vu process, resets state between files, executes steps.
...
// ---------------------------------------------------------------------------
// Vu process lifecycle
// ---------------------------------------------------------------------------

/// RAII guard for a running vu process.
/// Kills the process and removes the socket file on drop.
pub struct VuProcess {
    child: std::process::Child,
    socket: PathBuf,
}

impl VuProcess {
    /// Launch vu with a dedicated socket path and wait until the socket is ready.
    pub fn launch(vu_bin: &Path, socket: &Path, startup_timeout: Duration) -> Result<Self> {
        // Remove any stale socket from a previous run.
        let _ = std::fs::remove_file(socket);

        let child = Command::new(vu_bin)
            .env("VU_SOCKET_PATH", socket)
            // Suppress vu's own log output so it doesn't pollute test output.
            // Tests that need to inspect logs can override this.
            .env("RUST_LOG", "error")
            .spawn()
            .with_context(|| format!("failed to launch vu from {}", vu_bin.display()))?;

        let mut process = VuProcess {
            child,
            socket: socket.to_path_buf(),
        };

        process.wait_for_control_endpoint(vu_bin, startup_timeout)?;
        Ok(process)
    }

    /// Block until the control endpoint is ready or the timeout expires.
    fn wait_for_control_endpoint(&mut self, vu_bin: &Path, timeout: Duration) -> Result<()> {
        let deadline = Instant::now() + timeout;
        while Instant::now() < deadline {
            if let Some(status) = self.child_status()? {
                bail!("vu exited before the control endpoint was ready: {status}");
            }

            if control_endpoint_ready(&self.socket) {
                // Give vu a brief moment to finish installing the workspace before tests start.
        ...
        bail!(
            "vu ({}) did not expose control endpoint at {} within {:?}",
            vu_bin.display(),
            self.socket.display(),
            timeout
        )
    ...
    fn child_status(&mut self) -> Result<Option<std::process::ExitStatus>> {
        self.child
            .try_wait()
            .context("failed to poll vu child process status")
    }
}
...
#[cfg(windows)]
fn control_endpoint_ready(socket: &Path) -> bool {
    // Windows uses Named Pipes (e.g. \\.\pipe\vu). Opening as a file
...
/// Reset vu to a known baseline state before each test file:
///   1. Close all tabs except tab 1 (vu requires at least one tab).
...
pub fn reset_state(vu_cli: &Path, socket: &Path) -> Result<()> {
    reset_tabs(vu_cli, socket)?;
    reset_panes(vu_cli, socket)?;
    reset_surfaces(vu_cli, socket)?;
    Ok(())
}

fn reset_tabs(vu_cli: &Path, socket: &Path) -> Result<()> {
    loop {
        let json = cli_json(vu_cli, socket, &["tabs", "list"])?;
        let tabs = json["tabs"]
            .as_array()
...
fn reset_panes(vu_cli: &Path, socket: &Path) -> Result<()> {
    let mut attempts = 0usize;
    loop {
        let json = cli_json(vu_cli, socket, &["panes", "list", "--tab", "1"])?;
...
fn reset_surfaces(vu_cli: &Path, socket: &Path) -> Result<()> {
    let mut attempts = 0usize;
    loop {
        let json = cli_json(
            vu_cli,
...
/// Run a vu-cli command and return its stdout parsed as JSON.
fn cli_json(vu_cli: &Path, socket: &Path, args: &[&str]) -> Result<Value> {
    let socket_str = socket.to_string_lossy().into_owned();
    let mut full_args = vec!["--socket", &*socket_str, "--json"];
    full_args.extend_from_slice(args);
    let output = Command::new(vu_cli)
        .args(&full_args)
        .output()
        .with_context(|| format!("cli_json: failed to run vu-cli {:?}", args))?;
    if !output.status.success() {
        bail!(
            "cli_json: vu-cli {:?} failed: {}",
            args,
            String::from_utf8_lossy(&output.stderr)
        );
    }
    serde_json::from_slice(&output.stdout)
        .with_context(|| format!("cli_json: invalid JSON from vu-cli {:?}", args))
}

/// Run a vu-cli command, return error if exit code != 0.
fn cli_run(vu_cli: &Path, socket: &Path, args: &[&str]) -> Result<()> {
    let socket_str = socket.to_string_lossy().into_owned();
    ...
    let output = Command::new(vu_cli)
        .args(&full_args)
        .output()
        .with_context(|| format!("cli_run: failed to run vu-cli {:?}", args))?;
    if !output.status.success() {
        bail!(
            "cli_run: vu-cli {:?} failed: {}",
            args,
            String::from_utf8_lossy(&output.stderr)
        );
    }
...
fn run_step(step: &Step, config: &RunConfig) -> Result<StepOutcome> {
    let mut cmd = Command::new(&config.vu_cli);
    cmd.arg("--socket").arg(&config.socket);
    cmd.args(&step.args);

    let output = cmd
        .output()
        .with_context(|| format!("failed to run vu-cli for step {:?}", step.label))?;
...
fn exit_failure(step: &Step, cmd_display: &str, stderr: &str) -> StepFailure {
    StepFailure {
        ...
        cmd: cmd_display.to_string(),
        expected: step.expected.clone(),
        actual: format!("vu-cli exited with error\nstderr: {stderr}"),
        diff: String::new(),
    }
}
