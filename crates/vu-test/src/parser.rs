...
///
/// Both `vu-cli ...` (preferred) and legacy `cmd ...` step directives are
...
pub struct Step {
    ...
    pub label: String,
    /// Arguments to pass to vu-cli (already split on whitespace, respecting quotes)
    pub args: Vec<String>,
...
pub fn parse_source(source: &str, origin: String) -> Result<Vec<Step>> {
    ...
    while i < lines.len() {
        ...
        if let Some(rest) = step_args(trimmed) {
            ...
            // Split inline label comment: `vu-cli foo bar  # my label`
            let (args_str, label) = split_inline_comment(rest);
            let args = shell_split(args_str.trim()).with_context(|| {
                format!("{origin}:{cmd_line}: failed to parse vu-cli arguments")
            })?;
            ...
            if i >= lines.len() {
                bail!(
                    "{origin}:{}: expected `----` after vu-cli/cmd block, got <eof>",
                    i + 1
                );
            }
            ...
            let inline_mode_str = if sep_line == "----" {
                ...
                bail!(
                    "{origin}:{}: expected `----` after vu-cli/cmd block, got {:?}",
                    i + 1,
                    sep_line
                );
            };
            ...
            bail!(
                "{origin}:{}: unexpected line {:?} (expected `vu-cli`, legacy `cmd`, `#`, or blank)",
                i + 1,
                trimmed
            );
        }
...
fn step_args(line: &str) -> Option<&str> {
    line.strip_prefix("vu-cli ")
        .or_else(|| if line == "vu-cli" { Some("") } else { None })
...
mod tests {
    ...
    fn inline_label_comment() {
        let src = "vu-cli --json identify  # smoke test\n----\ncon\n";
        let steps = parse_source(src, "test".into()).unwrap();
