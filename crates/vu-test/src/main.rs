...
#[command(
    name = "vu-test",
    about = "Logic test runner for vu-cli — drives a live vu session via .test files"
)]
struct Cli {
    ...
    /// Path to the vu binary to launch (defaults to sibling in cargo target/)
    #[arg(long, value_name = "PATH")]
    vu: Option<PathBuf>,

    /// Path to the vu-cli binary (defaults to sibling in cargo target/)
    #[arg(long, value_name = "PATH")]
    vu_cli: Option<PathBuf>,

    /// Control endpoint path for the launched vu process
    /// (default: temp Unix socket on Unix, named pipe on Windows)
    #[arg(long, value_name = "PATH")]
    socket: Option<PathBuf>,

    /// Seconds to wait for vu to start (default: 30)
...
fn default_control_endpoint() -> PathBuf {
    #[cfg(windows)]
    {
        PathBuf::from(format!(r"\\.\pipe\vu-test-{}", std::process::id()))
    }

    #[cfg(not(windows))]
    {
        std::env::temp_dir().join(format!("vu-test-{}.sock", std::process::id()))
    }
}
...
fn main() -> ExitCode {
    ...
    let vu_bin = match resolve_bin(cli.vu.as_deref(), app_binary_name()) {
        Ok(p) => p,
        Err(e) => {
    ...
    let vu_cli = match resolve_bin(cli.vu_cli.as_deref(), "vu-cli") {
        Ok(p) => p,
        Err(e) => {
            eprintln!("{} {e}", color.red("error:"));
            return ExitCode::FAILURE;
    ...
    // Launch a single vu process for the entire test run.
    println!("{} vu ({})...", color.dim("launching"), vu_bin.display());
    let _vu_process = match runner::VuProcess::launch(
        &vu_bin,
        &socket,
        ...
        Ok(p) => p,
        Err(e) => {
            eprintln!("{} failed to launch vu: {e}", color.red("error:"));
            return ExitCode::FAILURE;
        }
    };
    println!(
        "{} {}\n",
        color.dim("vu ready on"),
    ...
    for path in &test_files {
        if let Err(e) = runner::reset_state(&vu_cli, &socket) {
            eprintln!(
...
fn resolve_bin(flag: Option<&Path>, name: &str) -> Result<PathBuf> {
    ...
    // Sibling in the same target directory as vu-test itself.
    let sibling = {
...
mod tests {
    ...
    fn env_keys_match_documented_overrides() {
        assert_eq!(env_key_for_bin("vu"), "VU");
        assert_eq!(env_key_for_bin("vu-app"), "VU");
        assert_eq!(env_key_for_bin("vu-cli"), "VU_CLI");
    }
    ...
    fn default_socket_endpoint_is_platform_appropriate() {
        ...
        #[cfg(windows)]
        assert!(endpoint.starts_with(r"\\.\pipe\vu-test-"));
        #[cfg(not(windows))]
    ...
    fn app_binary_name_is_platform_appropriate() {
        #[cfg(windows)]
        assert_eq!(app_binary_name(), "vu-app");
        #[cfg(not(windows))]
        assert_eq!(app_binary_name(), "vu");
    }

    #[test]
    fn sibling_lookup_uses_platform_exe_name() {
        #[cfg(windows)]
        assert_eq!(platform_exe_name("vu-cli"), "vu-cli.exe");
        #[cfg(not(windows))]
        assert_eq!(platform_exe_name("vu-cli"), "vu-cli");
    }

    #[test]
    fn explicit_flag_wins_over_env() {
        let _guard = EnvGuard::set("VU", "/tmp/ignored-vu");
        let resolved = resolve_bin(Some(Path::new("/tmp/explicit-vu")), "vu").unwrap();
        assert_eq!(resolved, PathBuf::from("/tmp/explicit-vu"));
    }

    #[test]
    fn documented_env_override_is_used() {
        let _guard = EnvGuard::set("VU_CLI", "/tmp/documented-vu-cli");
        let resolved = resolve_bin(None, "vu-cli").unwrap();
        assert_eq!(resolved, PathBuf::from("/tmp/documented-vu-cli"));
    }
