...
/// A bidirectional, blocking byte stream connected to the vu control
...
fn connect_control_endpoint(_path: &Path) -> std::io::Result<ControlStream> {
    Err(std::io::Error::new(
        ...
        "vu-cli is not supported on this target — see docs/impl/windows-port.md",
    ))
}
...
fn send_command(socket_path: &Path, command: ControlCommand) -> Result<Value> {
    let mut stream = connect_control_endpoint(socket_path).with_context(|| {
        format!(
            "failed to connect to vu at {}. Launch vu first or pass --socket/VU_SOCKET_PATH.",
            socket_path.display()
        )
    })?;
    ...
    if read == 0 {
        bail!("vu closed the control connection without returning a response");
    }

    let response: JsonRpcResponse =
        serde_json::from_str(&line).context("failed to decode vu control response")?;
    ...
    response
        .result
        .ok_or_else(|| anyhow::anyhow!("vu returned an empty control response"))
}
