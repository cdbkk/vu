...
Use this playbook to validate that Vu can keep one OpenCode pane alive across unrelated shell work and return to it for a later interactive turn.

## Environment

- Local machine
- Project root: `~/dev/temp/vu-bench-opencode-resume`
- Coding CLI: `opencode`
...
1. `Please prepare an OpenCode workspace in ~/dev/temp/vu-bench-opencode-resume by launching OpenCode there and preparing a paired local shell target for file, git, and test work.`
2. `Keep the OpenCode target prepared, but use its paired local shell target to create a tiny Python project in ~/dev/temp/vu-bench-opencode-resume with greet.py and test_greet.py, initialize git, make an initial commit, and run python3 -m unittest -q. Only touch the OpenCode pane if a blocking trust or continue prompt must be cleared first.`
