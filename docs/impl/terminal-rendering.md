...
vu now runs a single terminal runtime: embedded Ghostty on macOS.

vu does not maintain its own VT parser, PTY loop, scrollback grid, or canvas renderer anymore.
...
- one `GhosttyApp` is shared across the whole vu window
...
That split is intentional. Terminal correctness stays in Ghostty. Product intelligence stays in vu.

Pane zoom follows the same boundary. Zooming a pane is a Vu layout state,
...
- Vu should keep one shared `GhosttyApp` per window
...
At the app level, vu already does that. The remaining migration work is about replacing vu-specific split semantics with Ghostty-driven surface-tree semantics.

## Data flow

1. User input enters GPUI.
2. `GhosttyView` forwards keyboard and mouse events into `ghostty_surface_*`.
3. Ghostty uses its embedded runtime wakeup callback to schedule `ghostty_app_tick()` on the macOS main queue. Vu does not drive the core renderer from a fixed workspace polling loop anymore.
...
protocol. Vu handles these ingress paths separately:
...
Windows and Linux preview backends, Vu's portable renderers still need explicit
work before they can display Kitty graphics, Sixel, or iTerm2/OSC 1337 image
output.

## What vu reads from Ghostty

Today vu relies on these Ghostty-facing facts:
...
## What vu does not own anymore

The following responsibilities were removed from vu:
...
The host also must not invent viewport state that Ghostty has not emitted. Standalone Ghostty only scrolls its native container when a real scrollbar update exists. Vu now follows that rule too: before Ghostty publishes scrollbar data, the scroll container stays at its current position instead of being forced to a synthetic `offset = 0` state. That matters for TUI startup and reflow, where a fake top-of-history viewport can make the embed briefly paint from upper scrollback before Ghostty settles on the real bottom-anchored viewport.

macOS scroll input must also preserve Ghostty's scroll-event contract. AppKit precise scroll events are not keyboard modifier events; they must be sent with Ghostty's packed `ScrollMods.precision` bit and without scaling by the window backing factor. Vu mirrors Ghostty's AppKit host by sending precise deltas through that path with the same 2x multiplier, leaving Ghostty core to accumulate sub-row remainders and apply terminal-row scrolling.
...
If vu needs better pane intelligence, the right long-term move is:

1. consume stronger Ghostty facts when the C API exposes them
2. upstream missing observability hooks when needed
3. keep product inference in vu's pane runtime observer
