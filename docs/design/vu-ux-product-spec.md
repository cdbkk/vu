...
This document is the primary product and UX handoff for the next design phase of **vu**.
...
Those references inform the brief, but this document defines the design direction for **vu**, not for the reference products.

---

## Product Definition

**vu** is an open-source, native terminal emulator with a built-in AI harness.
...
### What vu should not be

vu should not feel like:
...
The best version of vu should combine:
...
vu must remain excellent even when the built-in agent is ignored.
...
vu should help structure work where it adds value, without replacing raw output.
...
Context quality is one of vu's biggest strategic opportunities.
...
This is where vu has to be more deliberate than many reference products.

### Core rule

These workflows should feel clearer in vu, not wrapped by vu.
...
- external agent CLIs can run directly in a pane without vu hijacking them
...
- When an external agent CLI is active, vu should support it with orientation, notifications, and safe approvals, not compete for control.
- Never let one address space masquerade as another. A vu pane, a tmux pane, and an editor buffer are different targets and must be surfaced that way.
...
vu should help users resume interrupted work gracefully.
...
The user runs a command, it fails, and vu offers quick routes to explain, fix, or ask about the result without breaking shell flow.
...
vu provides metadata, notifications, and smoother environment support without trying to replace the tool.
...
- How should vu represent remote risk without becoming noisy?
...
**vu is the terminal you trust enough to live in all day — fast enough for experts, calm enough for long sessions, and transparent enough for AI-native work.**
