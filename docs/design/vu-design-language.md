...
This document defines the design principles that should govern every prototype and implementation decision for **vu**.
...
1. **vu's product vision and early implementation**
   - `DESIGN.md`
...
The goal is not to imitate any one reference. The goal is to produce a design language that is unmistakably right for **vu**.

---

## Dual Theme Philosophy

vu is light-first (Flexoki Light default) but must excel in both themes.
...
vu should support serious workflows and high information density, but it must never feel packed or breathless.
...
Unlike a knowledge app or dashboard product, vu has to solve for:
