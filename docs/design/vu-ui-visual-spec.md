# vu UI and Visual Spec

## Purpose

This document defines the visual and interaction system for **vu**.
...
From the Nowledge Labs reference system, vu should inherit:
...
From strong terminal-native UX patterns, vu should inherit:
...
What makes vu distinct is that all of these qualities must be adapted to a **terminal-native** environment without losing polish in either theme.
...
Typography is one of the main carriers of polish in vu.
...
vu should adopt the same level of restraint while adapting it for a darker, more operational context.
...
Motion should make vu feel smoother and faster.
...
The React/Next.js prototype at `components/vu/terminal-app.tsx` serves as the production-ready design reference for GPUI implementation.
