...
vu is an open-source, GPU-accelerated terminal emulator. Built in Rust.
...
│   ├── vu-core/      # Shared logic (control plane, config, session)
...
settings, command palette, and control socket
...
- `skills/gpui-cache-aware/SKILL.md` — use when reviewing or changing UI
  performance, especially terminal-adjacent UI or
...
- **Font context-switching**: Use mono font (Ioskeley Mono) for shell/command contexts. Use system font (.SystemUIFont) for settings UI.
...
- Placeholders should be action-oriented and short: "Run a command…".
- Avoid numbered step wizards for simple 2-3 step flows — inline everything flat.

## Key Conventions

- **Crate boundaries matter.** vu-terminal has zero UI deps. vu-core glues the rest.
- **Control socket runtime.** The control socket uses a one-worker tokio runtime — no thread-per-message.
...
- **vu-test for integration** — use `vu-test` only for integration and interactive behavior that requires a running session (e.g., pane lifecycle, socket API).
